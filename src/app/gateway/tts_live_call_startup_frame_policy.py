...
# The first accepted response phrase now uses a one-step Qwen decoder chunk,
# which materializes 1,920 samples (80 ms at 24 kHz). A larger websocket frame
# would force that fast path to wait for another decoder chunk before any PCM
# could be handed off. Align the transport frame with the first raw chunk.
# Four-step later phrases still arrive as 7,680-sample raw chunks and are split
# into four steady 80 ms frames.
...
# chunk at 24 kHz. Arm the bounded non-zero fallback after that first decoder
# chunk so quiet but real speech can be handed off immediately. Exact digital
# silence is still rejected, and ordinary larger chunks retain the established
# 400 ms window.
