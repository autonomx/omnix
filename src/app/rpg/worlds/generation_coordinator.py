...
def _failed_topic_note(
    *,
    ...
    """Return durable, non-canon content for a terminal provider failure."""

    retry_note = {
        "status": "needs_retry",
        "message": (
            "This topic could not be generated by the configured provider. "
            "No deterministic fallback was published; retry it after the current "
            "world-generation run completes."
        ),
...
def _settings_from_payload(value: Mapping[str, Any]) -> WorldTopicGenerationSettings:
    return WorldTopicGenerationSettings(
        ...
        max_attempts=max(1, min(int(value.get("max_attempts") or 2), 2)),
    ...
    try:
        ...
        with unit_of_work(db) as work:
            ...
            if failed["status"] == "failed":
                failure_content = _failed_topic_note(
                    topic_id=topic_id,
                ...
                work.world_scenarios.put_topic(
                    ...
                    dependency_hashes=dict(payload.get("dependency_hashes") or {}),
                    input_hash=str(payload.get("input_hash") or ""),
                    content_hash=canonical_hash(failure_content),
                    provenance=dict(failure_content["provenance"]),
                )
            work.commit()
        if failed["status"] == "failed":
