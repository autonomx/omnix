...
def _unique(rows: Iterable[Mapping[str, Any]], *id_fields: str) -> list[dict[str, Any]]:
    values: dict[str, dict[str, Any]] = {}
    for index, row in enumerate(rows, start=1):
        item = dict(row)
        item_id = next((str(item.get(field) or "") for field in id_fields if item.get(field)), f"item:{index}")
        values.setdefault(item_id, item)
    return [values[key] for key in sorted(values)]


def _words(value: Any) -> tuple[str, ...]:
    text = "".join(ch.casefold() if ch.isalnum() else " " for ch in str(value or ""))
    return tuple(word for word in text.split() if len(word) >= 3)


def _document_cards(documents: list[dict[str, Any]], canon_revision: int) -> list[dict[str, Any]]:
    cards: list[dict[str, Any]] = []
    for document in documents:
        document_id = str(document.get("document_id") or "")
        title = str(document.get("title") or document_id)
        visibility = str(document.get("visibility") or "game_master_canon")
        entity_refs = [str(value) for value in document.get("entities") or () if str(value)]
        keywords = [str(value) for value in document.get("keywords") or () if str(value)]
        for size, field_name in (("short", "summary_120"), ("medium", "summary_500")):
...
def _fact_cards(facts: list[dict[str, Any]], canon_revision: int) -> list[dict[str, Any]]:
    cards: list[dict[str, Any]] = []
    for fact in facts:
        fact_id = str(fact.get("id") or fact.get("evidence_id") or "")
        content = str(fact.get("content") or fact.get("statement") or fact.get("object") or "").strip()
        ...
        cards.append(
            {
                ...
                "content": content,
                "authority": str(fact.get("approved_authority") or fact.get("authority") or "objective_canon"),
...
def _lexical_index(cards: list[dict[str, Any]]) -> dict[str, list[str]]:
    ...
    for card in cards:
        ...
        tokens.update(str(value).casefold() for value in card.get("keywords") or () if str(value))
        tokens.update(_words(value) for value in ())
        for token in tokens:
            index.setdefault(token, set()).add(card_id)
    return {token: sorted(card_ids) for token, card_ids in sorted(index.items())}


def _entity_index(entities: list[dict[str, Any]], cards: list[dict[str, Any]]) -> dict[str, Any]:
    cards_by_entity: dict[str, list[str]] = {}
    for card in cards:
        for entity_id in card.get("entity_refs") or ():
            cards_by_entity.setdefault(str(entity_id), []).append(str(card.get("id") or ""))
    return {
        str(entity.get("id")): {
            ...
            "retrieval_card_ids": sorted(set(cards_by_entity.get(str(entity.get("id")), []))),
        }
...
def _discovery_state(
    ...
    return {"pages": page_status, "entities": entity_status, "discoveries": []}
...
def _opening_ids(
    ...
    normalized = starting_location.casefold().replace("-", "_").replace(" ", "_")
    location_ids = {
        str(entity.get("id"))
        for entity in entities
        if entity.get("kind") == "location"
        and (
            normalized in str(entity.get("id") or "").casefold()
            or normalized in str(entity.get("name") or "").casefold().replace(" ", "_")
        )
    }
    if not location_ids:
        location_ids = {
        ...
        location_ids = set(sorted(location_ids)[:1])
    actor_ids = {
        str(entity.get("id"))
        for entity in entities
        if entity.get("kind") == "npc" and str(entity.get("location_id") or "") in location_ids
    }
    for thread in story_threads:
        if location_ids.intersection(str(value) for value in thread.get("location_ids") or ()):
            actor_ids.update(str(value) for value in thread.get("actor_ids") or () if str(value))
...
def compile_campaign_bible(
    ...
    *,
    ...
    """Compile topic outputs into human, atomic, ACL, and retrieval representations."""

    topics = generation.topics
    documents = _unique((row for topic in topics for row in topic.documents), "document_id", "id")
    entities = _unique((row for topic in topics for row in topic.entities), "id")
    facts = _unique((row for topic in topics for row in topic.facts), "id", "evidence_id")
    relationships = _unique(
        [*(row for topic in topics for row in topic.relationships), *(dict(row) for row in compiled_relationships)],
        "id",
    )
    knowledge_rules = _unique((row for topic in topics for row in topic.knowledge_rules), "id")
    story_threads = _unique((row for topic in topics for row in topic.story_threads), "id")
    for document in documents:
        document["canon_revision"] = canon_revision
    for row in [*facts, *relationships]:
        if row.get("authority") == "generated_proposal":
            row["authority"] = str(row.pop("approved_authority", "objective_canon"))
        row["canon_revision"] = canon_revision
    cards = _unique(
        [*_document_cards(documents, canon_revision), *_fact_cards(facts, canon_revision)],
        "id",
    )
    lexical = _lexical_index(cards)
    entity_index = _entity_index(entities, cards)
    opening_locations, opening_actors = _opening_ids(entities, story_threads, starting_location)
    entity_by_id = {str(row.get("id")): row for row in entities}
    missing: list[str] = []
    required_topics = set(str(value) for value in topic_graph.get("launch_required_topic_ids") or ())
    completed_topics = {topic.topic_id for topic in topics}
    for topic_id in sorted(required_topics.difference(completed_topics).difference({"relationships", "consistency_audit", "canon_compile", "retrieval_index", "opening_materialization"})):
    ...
    completeness = {
        "score": round(max(0.0, 1.0 - len(missing) / max(1, len(required_topics))), 4),
    ...
    retrieval_index = {
        ...
        "embedding_index": {"status": "not_built", "reason": "provider_independent_genesis"},
    }
    document = {
        ...
        "manifest": {
            ...
            "generation_order": [list(batch) for batch in generation.generation_order],
        },
        "documents": documents,
        "entities": {str(row["id"]): row for row in entities if row.get("id")},
        "facts": facts,
    ...
    return CanonCompilationResult(
        ...
        metadata={
            "compiler": "rpg_campaign_canon_compiler_v1",
