...
DOMAIN_SPECS: Mapping[str, DomainSpec] = {
    "points_of_interest": DomainSpec(
        "point_of_interest", "poi", ("name", "location_id", "region_id", "purpose", "sensory_profile"), ("hooks",),
        {"location_id": ("location",), "region_id": ("region",)},
    ),
    "quests": DomainSpec(
        "quest", "quest", ("name", "giver_id", "stakes"), ("location_ids", "faction_ids", "objectives", "rewards"),
        {"giver_id": ("npc",), "location_ids": ("location",), "faction_ids": ("faction",)},
    ),
    "monsters": DomainSpec(
        "monster", "monster", ("name", "threat_level"), ("region_ids", "habitats", "abilities", "weaknesses"),
        {"region_ids": ("region",)},
    ),
    "items": DomainSpec(
        "item", "item", ("name", "item_type", "rarity", "value"), ("effects", "origin_ids"),
        {"origin_ids": ("faction", "location")},
    ),
    "races": DomainSpec(
        "race", "race", ("name", "lifespan"), ("homelands", "cultures", "traits", "languages"),
        {"homelands": ("region",)},
    ),
    "classes": DomainSpec(
        "class", "class", ("name",), ("capabilities", "progression", "equipment", "institution_ids"),
        {"institution_ids": ("institution",)},
    ),
    "spells": DomainSpec(
        "spell", "spell", ("name", "school", "tier", "range"), ("costs", "effects"),
        {"institution_ids": ("institution",)},
    ),
    "feats": DomainSpec(
        "feat", "feat", ("name",), ("prerequisites", "benefits", "limitations"),
        {"class_ids": ("class",)},
    ),
    "encounter_seeds": DomainSpec(
        "encounter_seed", "encounter", ("name", "setup"), ("location_ids", "actor_ids", "threat_ids", "complications", "outcomes"),
        {"location_ids": ("location",), "actor_ids": ("npc",), "threat_ids": ("monster",)},
    ),
    "one_shots": DomainSpec(
        "one_shot", "one-shot", ("name", "premise"), ("location_ids", "actor_ids", "quest_ids", "beats", "rewards"),
        {"location_ids": ("location",), "actor_ids": ("npc",), "quest_ids": ("quest",)},
    ),
    "opening_scenarios": DomainSpec(
        "opening_scenario", "opening", ("name", "starting_location_id", "premise"), ("initial_npc_ids", "opening_seed_ids", "starting_resources"),
        {"starting_location_id": ("location",), "initial_npc_ids": ("npc",), "opening_seed_ids": ("quest", "one_shot", "encounter_seed")},
    ),
}
...
def _ids_by_kind(known: Mapping[str, Mapping[str, Any]], kinds: Sequence[str]) -> list[str]:
    allowed = set(kinds)
    return sorted(
        entity_id
...
def _generic_name(node: CampaignTopicNode, index: int) -> str:
    singular = node.title.rstrip("s")
    return f"{singular} {index + 1}"
...
def _default_scalar(
    ...
    *,
    ...
    if field in spec.reference_fields:
        candidates = _ids_by_kind(known, spec.reference_fields[field])
    ...
    defaults: Mapping[str, Any] = {
        ...
        "value": (index + 1) * 25,
        "lifespan": "Varies by lineage and environment; adulthood and aging are culturally documented.",
        "school": ["evocation", "warding", "divination", "transmutation"][index % 4],
        "tier": index % 5 + 1,
        "range": ["self", "touch", "near", "far"][index % 4],
        "setup": f"The party encounters {name} while another faction or creature acts under time pressure.",
        "premise": f"A self-contained adventure centered on {name}, with a clear choice and lasting consequence.",
    }
    return defaults.get(field, f"Defined {field.replace('_', ' ')} for {name}.")
...
def _default_list(
    ...
    *,
    ...
    if field == "starting_resources":
        return [{"resource": "currency", "amount": 25}, {"resource": "rations", "amount": 3}]
    defaults: Mapping[str, list[str]] = {
        ...
        "abilities": ["one signature attack", "one environmental adaptation"],
        "weaknesses": ["a discoverable behavioral or material weakness"],
        "effects": ["one explicit mechanical benefit", "one narrative consequence"],
        "traits": ["one physical adaptation", "one social tradition"],
        "languages": ["Common", "a regional language"],
        "cultures": ["a named regional culture"],
        "progression": ["novice", "adept", "master"],
        "capabilities": ["one exploration capability", "one conflict capability"],
...
def _normalize_reference(
    ...
    *,
    ...
    valid = _ids_by_kind(known, allowed_kinds)
    values = value if isinstance(value, list) else [value] if value else []
    resolved = [str(item) for item in values if str(item) in known and str(known[str(item)].get("kind") or "") in set(allowed_kinds)]
    if not resolved and valid:
        resolved = [valid[0]]
...
def _normalize_entity(
    ...
    *,
    ...
    entity = dict(row)
    name = str(entity.get("name") or entity.get("title") or _generic_name(node, index)).strip()
    entity_id = str(entity.get("id") or entity.get("entity_id") or f"{spec.id_prefix}:{_slug(name)}").strip()
    if not entity_id.startswith(f"{spec.id_prefix}:"):
        entity_id = f"{spec.id_prefix}:{_slug(entity_id)}"
    entity.update({"id": entity_id, "name": name, "kind": spec.kind, "visibility": str(entity.get("visibility") or node.visibility)})
    for field in spec.required_scalars:
        value = entity.get(field)
        if field in spec.reference_fields:
            value = _normalize_reference(value, allowed_kinds=spec.reference_fields[field], known=known, multiple=False)
        if value in (None, ""):
            value = _default_scalar(node.topic_id, field, name=name, index=index, known=known, spec=spec)
        entity[field] = value
    for field in spec.required_lists:
        value = entity.get(field)
        if field in spec.reference_fields:
            value = _normalize_reference(value, allowed_kinds=spec.reference_fields[field], known=known, multiple=True)
        elif field == "starting_resources" and isinstance(value, Mapping):
            value = [dict(value)]
        elif not isinstance(value, list):
            value = [value] if value not in (None, "") else []
        if not value:
            value = _default_list(node.topic_id, field, name=name, index=index, known=known, spec=spec)
        entity[field] = value
    entity.setdefault("description", f"{name} is structured {spec.kind.replace('_', ' ')} canon for the campaign world.")
    entity.setdefault("schema_version", str(node.metadata.get("schema_version") or f"rpg_world_{node.topic_id}_v1"))
    return entity
...
def normalize_structured_domain(
    ...
    source = list(topic.entities)
    while len(source) < node.target_count:
        source.append({"name": _generic_name(node, len(source))})
    entities = tuple(
        _normalize_entity(node, row, index=index, known=known)
        for index, row in enumerate(source[: node.target_count])
    )
    ...
    return replace(
        ...
        provenance={
            **dict(topic.provenance),
            "domain_contract": str(node.metadata.get("schema_version") or f"rpg_world_{node.topic_id}_v1"),
            "domain_normalized": True,
            "domain_entity_ids": sorted(entity_ids),
        },
    )
...
def validate_structured_domain(
    ...
    if len(topic.entities) < node.target_count:
        raise ValueError(f"structured_domain_count:{node.topic_id}:{len(topic.entities)}:{node.target_count}")
    known = _known_entities(dependency_topics)
    seen: set[str] = set()
    for entity in topic.entities:
        ...
        for field, kinds in spec.reference_fields.items():
            ...
            for value in values:
                if known and str(value) not in known:
                    raise ValueError(f"structured_domain_reference:{node.topic_id}:{entity_id}:{field}:{value}")
                if str(value) in known and str(known[str(value)].get("kind") or "") not in set(kinds):
                    raise ValueError(f"structured_domain_reference_kind:{node.topic_id}:{entity_id}:{field}:{value}")
...
def generate_deterministic_domain(
    ...
    *,
    ...
    for index in range(node.target_count):
        adjective = ["Ashen", "Silver", "Hidden", "Storm", "Verdant", "Glass"][rng.randrange(6)]
        noun = node.title.split(" and ", 1)[0].rstrip("s")
        entities.append({"name": f"{adjective} {noun} {index + 1}"})
    normalized = normalize_structured_domain(
        node,
        GeneratedTopic(
            topic_id=node.topic_id,
            entities=tuple(entities),
            provenance={"generator": "deterministic_structured_domain_v1", "template": template},
        ),
        dependency_topics,
    )
