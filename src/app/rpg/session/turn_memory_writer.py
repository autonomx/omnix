...
def _facts(text: str) -> list[dict[str, str]]:
    ...
    if not match:
        return []
    value = re.split(r"\s+(?:and|but|because|when|while)\s+", match.group("value"))[0].strip().strip(".?!,;:\"'")[:48]
...
def _npc(result: Mapping[str, Any] | None) -> dict[str, str]:
    ...
    for candidate in (d(result_dict.get("npc")), d(nested.get("npc"))):
        speaker = first(candidate.get("speaker"), candidate.get("name")); npc_id = first(candidate.get("id"), candidate.get("npc_id"))
...
def write_turn_memory(session: Mapping[str, Any] | None, result: Mapping[str, Any] | None, *, player_input: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    tick = i(result_dict.get("tick"), i(runtime.get("tick"), 0)); turn_id = first(result_dict.get("turn_id"), f"turn:{tick}"); facts = _facts(player_input)
    ...
    memory["recent_turns"] = bounded([*memory["recent_turns"], turn], RECENT_TURN_LIMIT); dialogue = None
    if npc["id"] or facts:
