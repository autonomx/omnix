        ...
        *,
        ...
        unresolved = tuple(
            sorted(
                ...
                if _normalize_name(phrase) not in entities | locations
            )
        )
        ...
        if token_set & _TECH_WORDS:
            hypotheses.append(
                IntentHypothesis(
                    "use_impossible_technology",
                    "world_equivalent",
                    0.96,
                    _technology_goal(token_set),
                    ("world_rules", "available_services"),
                )
            )
        if token_set & _MAGIC_WORDS and not _mechanic_supported("magic", supported_mechanics):
            hypotheses.append(
                IntentHypothesis(
                    "use_unsupported_power",
                    "analogous_skill",
                    0.9,
                    "obtain information or influence through an unsupported power",
                    ("skills", "magic_rules", "target_state"),
                )
            )
        if token_set & _TRAVEL_WORDS:
            ...
            hypotheses.append(
                IntentHypothesis(
                    "travel",
                    "ask_directions" if unknown_destination else "travel_attempt",
                    0.82 if unknown_destination else 0.92,
                    "reach the requested destination",
                    ("known_locations", "routes", "travel_rules"),
                )
            )
        if unresolved:
            hypotheses.append(
                IntentHypothesis(
                    "reference_unknown_entity",
                    "entity_search",
                    0.84,
                    "identify or learn about the referenced entity",
                    ("entity_aliases", "npc_memory", "lorebook"),
                    ambiguity="medium",
                )
            )
        if (tokens and tokens[0] in _QUESTION_WORDS) or text.endswith("?"):
            hypotheses.append(
                IntentHypothesis(
                    ...
                    "lore_search",
                    0.88,
                    "obtain an answer consistent with available knowledge",
                    ("speaker_knowledge", "journal", "lorebook"),
                )
            )
        if token_set & _SOCIAL_WORDS:
            ...
            hypotheses.append(
                IntentHypothesis(
                    "social_action",
                    "clarification" if ambiguous else "social_check",
                    0.7 if ambiguous else 0.86,
                    "change another character's understanding or behavior",
                    ("target", "relationship", "social_mechanics"),
        ...
        if token_set & _ASSERTION_MARKERS and any(
            ...
            hypotheses.append(
                IntentHypothesis(
                    ...
                    0.8,
                    "use an asserted relationship or prior event",
                    ("campaign_history", "npc_memory", "relationships"),
                    ambiguity="medium",
                )
            )
        if not hypotheses:
            hypotheses.append(
                IntentHypothesis(
                    ...
                    "inspect_or_clarify",
                    0.45,
                    "understand what outcome the player is seeking",
