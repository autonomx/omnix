...
from .evidence import EvidenceAccessContext, EvidenceBroker, EvidenceQuery, EvidenceRetrievalResult
from .planner import DeterministicBeatPlanner, NarrativePlan
from .repository import InMemoryNarrativeResponseRepository, NarrativeResponseRepository
...
class NarrativeEngineService:
    ...
    def generate(self, request: TurnPresentationRequest) -> NarrativeEngineResult:
        ...
        validated = write_validate_repair(request, plan, evidence, self.writer)
    ...
    def _query(request: TurnPresentationRequest) -> EvidenceQuery:
        ...
        return EvidenceQuery(
            ...
            access=EvidenceAccessContext(
                ...
                faction_ids=tuple(request.metadata.get("faction_ids") or ()),
                narrator_mode=True,
            ),
        )
    ...
    def _assemble(
        ...
        revision = max(1, int(request.metadata.get("presentation_revision") or 1))
        response_id = str(
        ...
        return CanonicalNarrativeResponse(
            ...
            delivery=DeliveryMetadata(mode=DeliveryMode.BLOCKING, status="approved"),
            metadata={
                "mode": plan.mode,
