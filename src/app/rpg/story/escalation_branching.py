...
def seed_escalation_arcs(
    *,
    existing_arcs: Mapping[str, Any],
    escalation_hooks: Iterable[Mapping[str, Any]],
    turn_index: int,
    max_active_escalations: int = 2,
) -> Dict[str, Any]:
    arcs = {
    ...
    hooks = sorted(
        [_safe_dict(hook) for hook in _safe_list(list(escalation_hooks))],
        key=lambda item: int(item.get("priority") or 0),
        reverse=True,
    )

    for hook in hooks:
        if len(active_escalations) >= max_active_escalations:
            break
        ...
        arcs[arc_id] = {
            ...
            "history": [
                {
                    "turn": turn_index,
                    "type": "escalation_arc_seeded",
                    "summary": hook.get("summary", ""),
                }
            ],
        }

        active_escalations.append(arcs[arc_id])

        seeded_events.append(
            {
                "type": "story_arc",
                "subtype": "escalation_arc_seeded",
                "arc_id": arc_id,
                "source_hook_id": hook.get("id"),
                "summary": hook.get("summary", ""),
