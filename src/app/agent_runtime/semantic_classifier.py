...
def _system_prompt() -> str:
    return (
        ...
        "contains labeled Previous conversation context and Latest user steering sections, "
        "the latest steering is authoritative. The prior context can span multiple user and "
        "assistant turns; resolve references such as 'it', 'that issue', or 'fix it' against "
        "the most plausible earlier subject even when it was several messages back. Use "
        "previous conversation context only to resolve references or omitted subjects, and "
        "never preserve an action that the latest "
