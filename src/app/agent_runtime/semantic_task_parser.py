...
def _system_prompt() -> str:
    return (
        ...
        "says check, find, relevant, latest, narrow, cite, citations, or compare. A targeted "
        "verification pass that adds citations to already-supplied recommendations is bounded "
        "when it only needs the current authoritative documentation for those claims. Relating "
        "one current quote to a catalyst already established in reference_context is also "
        "bounded: read the quote and use conversation explain/compose semantics for the "
        "relationship; do not re-research the established catalyst. Narrowing an already-"
        "discovered candidate set using one current status/liquidity/volume snapshot is bounded "
        "as well. In all of these cases use autonomous=false and multi_step=false even when the "
        "answer contains comparison or synthesis. By contrast, asking whether any new official "
        "changes, announcements, incidents, catalysts, or other events occurred over a time "
        "window is investigation over an unknown result/source set: use research/compare, "
        "autonomous=true, multi_step=true even if the user asks one final question. Open-ended "
        "investigation or genuine multi-source synthesis likewise uses research/compare and "
        "multi_step=true. Set autonomous=true for requested open-ended work or state/file "
        "changes. data_dependencies describe only information the task actually depends on, "
        "whether required, and whether current; do not name tools or evidence source classes. "
        "Do not create a fresh external dependency for a fact already established in "
        "reference_context unless the latest user asks to re-check or refresh that fact. "
