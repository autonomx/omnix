...
class GapPullbackConfig(BaseModel):
    ...
    direct B1/VWAP break.  The v2-only timing/recovery fields below are ignored
    by all 1.x evaluators so persisted 1.x behavior remains unchanged.
    """
    ...
    # 2.0.0-only causal failed-selloff geometry.  These defaults are the frozen
    # V11 prospective candidate selected before external validation.
