...
def _fill_price(open_price: Decimal, side: str, slippage_bps: Decimal) -> Decimal:
    ...
    return open_price * (Decimal("1") + adjustment if side == "buy" else Decimal("1") - adjustment)
    ...
    *,
    ...
    if len(bars) < minimum:
        ...
        return BacktestRunResult(
            ...
            trades=(),
            equity_curve=(),
            logs=(BacktestLogEntry(log_index=0, level="error", message="insufficient dataset history"),),
    ...
    for index, bar in enumerate(bars):
        if pending is not None:
            ...
            if side == "buy" and position == 0:
                ...
                commission_rate = request.execution_policy.commission_bps / Decimal("10000")
                quantity = budget / (open_price * (Decimal("1") + commission_rate))
                notional = quantity * open_price
                commission = _commission(notional, request.execution_policy.commission_bps)
