"""SQLite schema for backend-owned Chat memory."""
...
ON memory_records(scope, scope_id, status, pinned, updated_at);

CREATE INDEX IF NOT EXISTS idx_memory_records_normalized
ON memory_records(scope, scope_id, normalized_content);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_memory_snapshots_session_revision
...
def initialize_schema(connection: sqlite3.Connection) -> None:
    ...
    if int(row[0]) != SCHEMA_VERSION:
        raise RuntimeError(
            f"unsupported assistant memory schema version: {row[0]} (expected {SCHEMA_VERSION})"
        )
