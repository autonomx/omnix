...
_INDEXES = """
CREATE INDEX IF NOT EXISTS idx_memory_records_owner_scope
ON memory_records(owner_type, owner_id, scope, scope_id, status, pinned, updated_at);
CREATE INDEX IF NOT EXISTS idx_memory_records_normalized
ON memory_records(owner_type, owner_id, scope, scope_id, normalized_content);
CREATE INDEX IF NOT EXISTS idx_memory_candidates_owner_status
ON memory_candidates(owner_type, owner_id, status, created_at, id);
CREATE UNIQUE INDEX IF NOT EXISTS idx_memory_snapshots_owner_revision
ON memory_snapshots(session_id, owner_type, owner_id, revision);
CREATE INDEX IF NOT EXISTS idx_memory_snapshot_items_record
ON memory_snapshot_items(memory_record_id, revoked_at);
CREATE INDEX IF NOT EXISTS idx_memory_events_entity
ON memory_events(entity_type, entity_id, id);
"""
...
def initialize_schema(connection: sqlite3.Connection) -> None:
    connection.execute("CREATE TABLE IF NOT EXISTS memory_schema_version (version INTEGER NOT NULL)")
