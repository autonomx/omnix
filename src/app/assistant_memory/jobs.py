...
def create_memory_suggestion_job_request(session_id: str, user_message_id: str) -> CreateJobRequest:
    ...
    return CreateJobRequest(
        ...
        input_payload=MemorySuggestionJobInput(session_id=session_id, user_message_id=user_message_id, idempotency_key=key).model_dump(mode="json"),
...
def process_memory_suggestion_job(job: JobRecord, *, chat_store: ChatSessionStore, memory_service: MemoryService | None = None, job_store: SQLiteJobStore | None = None) -> MemorySuggestionJobResult:
    ...
    if session is None:
        ...
        result = MemorySuggestionJobResult(job_id=job.id, skipped_reasons=["character_memory_write_disabled"])
    else:
        ...
        if message is None:
            ...
            for item in extracted:
                candidate = service.propose_memory(
                    ...
                    scope=item["scope"], category=item["category"], content=item["content"], confidence=item["confidence"],
