...
def voice_clone_sources() -> list[tuple[Path, Path]]:
    ...
    if override_dir:
        ...
        return [(clones_dir, Path(override_file) if override_file else clones_dir / "voice_clones.json")]

    canonical_dir = resources_root() / "voice_clones"
    sources: list[tuple[Path, Path]] = [(canonical_dir, Path(override_file) if override_file else canonical_dir / "voice_clones.json")]
...
def discover_voice_clone_assets() -> list[AssetRecord]:
    ...
    for clones_dir, manifest_path in voice_clone_sources():
        ...
        for legacy_name, data in sorted(manifest_entries.items()):
            ...
            audio_path = _resolve_audio_path(clones_dir, clone_id, data)
    ...
    if isinstance(raw, list):
        ...
        for voice_id, payload in raw.items():
            if isinstance(payload, dict):
                ...
                entries[str(voice_id)] = {"profile_name": str(voice_id), "source_path": payload}
    return entries


def _resolve_audio_path(clones_dir: Path, clone_id: str, data: dict[str, Any]) -> Path | None:
    ...
    if clone_name:
        ...
        if clones_dir.is_dir():
            for suffix in _AUDIO_MIME_TYPES:
                candidates.extend(clones_dir.rglob(f"{stem}{suffix}"))
...
def _read_sidecar(audio_path: Path) -> tuple[Path | None, dict[str, Any]]:
    ...
    for sidecar_path in candidates:
        if not sidecar_path.is_file() or sidecar_path.name == "voice_clones.json":
            continue
        try:
