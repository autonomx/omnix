...
(function () {
    ...
    let storyState = {
        ...
        audioSegments: [],   // base64 WAV chunks as they arrive (SSE fallback)
        audioPcmChunks: [],  // raw PCM chunks as they arrive (WebSocket path)
        combinedAudioUrl: null,
        abortController: null,
    };

    // Streaming playback state – play chunks/segments as they arrive
    ...
    function _resetStoryState() {
        if (storyState.combinedAudioUrl) {
            try { URL.revokeObjectURL(storyState.combinedAudioUrl); } catch (_) { /* ignore */ }
        }
    ...
    function _abortGeneration() {
        if (storyState.abortController) {
            try { storyState.abortController.abort(); } catch (_) { /* ignore */ }
    ...
    function _makeAbortError(message) {
        const err = new Error(message || 'Aborted');
        err.name = 'AbortError';
        return err;
    }

    function _normaliseVoiceMap(map) {
        const out = {};
        Object.entries(map || {}).forEach(([speaker, voice]) => {
            if (!speaker || !voice) return;
            out[speaker] = voice;
            out[String(speaker).toLowerCase().trim()] = voice;
        });
        return out;
    }
...
<div class="story-speaker-row" data-speaker="${_esc(sp.name)}">
  ...
  <select class="story-voice-select" onchange="setStoryVoice('${_esc(sp.name)}', this.value)">
    ...
    // ---------------------------------------------------------------------------
    // Streaming playback – play WebSocket PCM chunks or SSE segments immediately
    // ---------------------------------------------------------------------------
    ...
    function _ensurePlaybackContext() {
        if (!_playbackCtx || _playbackCtx.state === 'closed') {
            _playbackCtx = new (window.AudioContext || window.webkitAudioContext)({ sampleRate: 24000 });
            _playbackEndTime = _playbackCtx.currentTime;
        }
        if (_playbackCtx.state === 'suspended') {
            _playbackCtx.resume().catch(() => { /* ignore */ });
        }
        return _playbackCtx;
    }

    function _playPcmChunkImmediately(arrayBuffer, sampleRate) {
        try {
            const ctx = _ensurePlaybackContext();
            const pcm16 = new Int16Array(arrayBuffer);
            if (!pcm16.length) return;

            const audioBuffer = ctx.createBuffer(1, pcm16.length, sampleRate || 24000);
            const channel = audioBuffer.getChannelData(0);
            for (let i = 0; i < pcm16.length; i++) {
                channel[i] = Math.max(-1, Math.min(1, pcm16[i] / 32768));
            }

            const source = ctx.createBufferSource();
            source.buffer = audioBuffer;
            source.connect(ctx.destination);
            const startAt = Math.max(ctx.currentTime + 0.02, _playbackEndTime);
            source.start(startAt);
            _playbackEndTime = startAt + audioBuffer.duration;
        } catch (e) {
            console.warn('[STORY] Failed to play PCM chunk immediately:', e);
        }
    }
    ...
    async function _playSegmentImmediately(b64) {
        ...
        try {
            ...
            source.connect(ctx.destination);
            const startAt = Math.max(ctx.currentTime, _playbackEndTime);
            source.start(startAt);
    ...
    async function generateStoryAudiobook() {
        ...
        const progressText = _el('story-progress-text');
        if (progressBar) progressBar.style.width = '0%';
        if (progressText) progressText.textContent = 'Starting audio stream…';

        try {
            try {
                await _generateStoryAudiobookWS(segments);
            } catch (wsErr) {
                if (wsErr && wsErr.name === 'AbortError') throw wsErr;

                console.warn('[STORY] WebSocket audio failed, falling back to SSE:', wsErr);
                storyState.audioPcmChunks = [];
                _stopStreamingPlayback();

                if (progressBar) progressBar.style.width = '0%';
                if (progressText) progressText.textContent = 'WebSocket unavailable; falling back to SSE…';

                await _generateStoryAudiobookSSE(segments);
            }
        } catch (err) {
    ...
    // ---------------------------------------------------------------------------
    // WebSocket Story Teller audio generation – raw PCM chunks play immediately
    // ---------------------------------------------------------------------------
    function _generateStoryAudiobookWS(segments) {
        const wsProtocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
        const wsUrl = `${wsProtocol}//${window.location.host}/ws/audiobook`;
        const jobId = 'story_' + Date.now() + '_' + Math.random().toString(36).slice(2, 8);
        ...
        return new Promise((resolve, reject) => {
            ...
            const aborter = {
                abort: () => {
                    ...
                    reject(_makeAbortError('Story audiobook generation aborted'));
                }
            };
            storyState.abortController = aborter;

            ws.onopen = () => {
                ws.send(JSON.stringify({
                    type: 'start',
                    segments,
                    voice_mapping: _normaliseVoiceMap(storyState.voiceMapping),
            ...
            ws.onmessage = (event) => {
                if (event.data instanceof ArrayBuffer) {
                    ...
                    storyState.audioPcmChunks.push(chunk);
                    _playPcmChunkImmediately(chunk, sampleRate);
                    return;
                }
                ...
                try {
                    ...
                    console.warn('[STORY] Ignoring malformed WS event:', err);
                    return;
                }
                ...
                if (data.type === 'start') {
                    if (Number.isFinite(data.total_segments) && data.total_segments > 0) {
                        totalSegments = data.total_segments;
                    }
                    if (progressText) progressText.textContent = '🔊 Streaming audio…';
                } else if (data.type === 'segment') {
                    lastSegmentIndex = Number.isFinite(data.index) ? data.index : (lastSegmentIndex + 1);
                    const completed = Math.max(0, lastSegmentIndex + 1);
                    const pct = Math.min(99, Math.round((completed / Math.max(1, totalSegments)) * 100));
                    if (progressBar) progressBar.style.width = pct + '%';
                    if (progressText) {
                        const speaker = data.speaker || 'Narrator';
                        progressText.textContent = `🔊 Playing… generated ${completed} / ${totalSegments} segments (${speaker})`;
                    }
                } else if (data.type === 'done') {
                    finished = true;
                    if (progressBar) progressBar.style.width = '100%';
                    if (progressText) progressText.textContent = `Done! ${storyState.audioPcmChunks.length} audio chunks streamed.`;
                    if (storyState.abortController === aborter) storyState.abortController = null;
                    try { ws.close(); } catch (_) { /* ignore */ }
                    _buildAndShowPcmAudio();
                    resolve();
                } else if (data.type === 'stopped') {
                    fail(_makeAbortError('Story audiobook generation stopped'));
                } else if (data.type === 'error') {
                    fail(new Error(data.message || data.error || 'WebSocket audio error'));
                }
            };

            ws.onerror = () => {
                fail(new Error('WebSocket audio stream failed'));
            };

            ws.onclose = () => {
                if (!finished) {
                    fail(new Error('WebSocket audio stream closed unexpectedly'));
                }
            };
        });
    }
    ...
    // ---------------------------------------------------------------------------
    // Legacy SSE fallback – base64 chunks play one segment at a time
    // ---------------------------------------------------------------------------
    async function _generateStoryAudiobookSSE(segments) {
    ...
    // ---------------------------------------------------------------------------
    function _buildAndShowPcmAudio() {
        const chunks = storyState.audioPcmChunks;
        if (!chunks.length) return;

        const wavBlob = _pcmChunksToWavBlob(chunks, 24000);
    ...
    // WAV encoders
    // ---------------------------------------------------------------------------
    function _pcmChunksToWavBlob(chunks, sampleRate) {
        const normalised = chunks.map(ch => ch instanceof Uint8Array ? ch : new Uint8Array(ch));
        const dataSize = normalised.reduce((sum, ch) => sum + ch.byteLength, 0);
        ...
        return new Blob([header].concat(normalised), { type: 'audio/wav' });
    }
