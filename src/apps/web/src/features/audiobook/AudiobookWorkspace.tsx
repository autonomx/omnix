...
export function AudiobookWorkspace({ module }: { module: OmnixModuleDefinition }) {
  ...
  function workspaceDocumentList(): WorkspaceDocument[] {
    ...
    return [
      ...
      { id: 'outline', title: 'Chapter outline (generated)', type: 'Outline', lastEdited: 'Generated', author: 'Omnix', linked: `All (${project.chapters.length})`, status: 'Generated', preview: project.chapters.map((chapter) => `${chapter.ordinal + 1}. ${chapter.title}`).join('\n') || 'No chapters have been extracted yet.' },
      { id: 'pronunciation-guide', title: 'Pronunciation guide (generated)', type: 'Pronunciation', lastEdited: 'Generated', author: 'Omnix', linked: `All (${project.chapters.length})`, status: 'Generated', preview: project.pronunciations?.map((item) => `${item.source_term} → ${item.spoken_term}`).join('\n') || 'Add pronunciations from the Pronunciations workspace.' },
      { id: 'review-notes', title: 'Review summary (generated)', type: 'Production', lastEdited: 'Generated', author: 'Omnix', linked: '—', status: 'Generated', preview: project.review_issues.length ? `${project.review_issues.length} review issues remain before rendering.` : 'No open review issues.' },
      ...(exportsQuery.data?.exports ?? []).map((item) => ({ id: `export-${item.id}`, title: `${item.format.toUpperCase()} Export Report`, type: 'Export Notes', lastEdited: new Date(item.created_at).toLocaleString(), author: 'Omnix', linked: 'Entire book', status: 'Final' as const, preview: `Manifest ${item.manifest_hash}`, href: `${base}/projects/${encodeURIComponent(project.id)}/exports/${encodeURIComponent(item.id)}/report` })),
    ];
  }
  ...
  function openDocument(document: WorkspaceDocument): void {
    if (document.id === 'outline') navigateLibrary('chapters');
    else if (document.id === 'pronunciation-guide') navigateLibrary('pronunciations');
    else if (document.id.startsWith('export-')) navigateLibrary('exports');
        ...
        {project && <>
              ...
              {project.chapters.filter((chapter) => chapter.title.toLowerCase().includes(bookSearch.toLowerCase())).map((chapter) => {
                ...
                return <article className="audiobook-book-card" key={chapter.id}>
                  ...
                  <div className="audiobook-book-card-copy"><small>CHAPTER {chapter.ordinal + 1}</small><h3>{chapter.title}</h3><span>{chapter.character_count.toLocaleString()} characters · {render?.status ?? 'Draft'}</span>
                    <div><button type="button" onClick={() => { setChapterId(chapter.id); navigateLibrary('projects'); }}>Continue</button><button type="button" disabled={busy || !modelRevision.trim()} onClick={() => void action(async () => { const detail = chapter.id === selectedChapter?.id ? selectedChapter : await queryClient.fetchQuery({ queryKey: ['audiobook', 'chapter', project.id, chapter.id], queryFn: () => omnixApiClient.get<Chapter>(`${base}/projects/${encodeURIComponent(project.id)}/chapters/${encodeURIComponent(chapter.id)}`) }); const span = detail.spans[0]; if (!span) throw new Error('This chapter has no speech spans yet.'); await omnixApiClient.post(`${base}/projects/${encodeURIComponent(project.id)}/preview`, { chapter_id: chapter.id, span_id: span.id, model_revision: modelRevision.trim() }); setChapterId(chapter.id); setSelectedSpanId(span.id); navigateLibrary('projects'); }, 'Chapter preview queued.')}>Play</button></div>
                  </div>
                </article>;
              })}
            </div>
          ...
          {workspaceMode === 'review' && librarySection === 'chapters' && (() => {
            ...
            return <section className="audiobook-card audiobook-chapters-panel">
              ...
              {chapterView === 'list' ? <div className="audiobook-chapter-table" role="table" aria-label="Audiobook chapters"><div className="audiobook-chapter-table-head" role="row"><span>#</span><span>Chapter title</span><span>Status</span><span>Scenes</span><span>Words</span><span>Est. runtime</span><span>Narrator / voice</span><span>Review</span><span>Actions</span></div>{chapters.map((chapter) => { const status = chapterStatus(chapter); const issues = project.review_issues.filter((issue) => issue.chapter_id === chapter.id); return <div className="audiobook-chapter-table-row" role="row" key={chapter.id}><span className="chapter-number">{chapter.ordinal + 1}</span><button type="button" className="chapter-title-button" onClick={() => startChapter(chapter)}><strong>{chapter.title}</strong><small>{chapter.character_count.toLocaleString()} characters</small></button><span className={`audiobook-status-pill status-${status.toLowerCase().replaceAll(' ', '-')}`}>{status}</span><span>—</span><span>{chapter.word_count?.toLocaleString() ?? '—'}</span><span>{chapter.word_count === undefined ? '—' : formatDuration(chapter.word_count / 150 * 60)}</span><span>{project.speakers[0]?.canonical_name ?? 'Narrator'}<small>{project.speakers[0]?.casting ? 'Voice assigned' : 'Needs voice'}</small></span><span className={issues.length ? 'review-pending' : 'review-approved'}>{issues.length ? `${issues.length} open` : ['ready_to_render', 'rendering', 'mastering', 'ready_to_export', 'exported'].includes(project.state) ? 'Approved' : 'Pending'}</span><div className="audiobook-row-actions"><button type="button" aria-label={`Preview first span of ${chapter.title}`} disabled={busy || !modelRevision.trim()} onClick={() => void action(() => queueChapterPreview(chapter), 'Chapter preview queued.')}>▶</button></div></div>; })}</div> : <div className="audiobook-chapter-grid">{chapters.map((chapter) => <article className="audiobook-chapter-grid-card" key={chapter.id}><div className="audiobook-chapter-grid-cover">{project.cover_asset_id && <img src={`${base}/projects/${encodeURIComponent(project.id)}/cover`} alt="" />}<b>{chapter.ordinal + 1}</b></div><div><span className="audiobook-status-pill">{chapterStatus(chapter)}</span><h3>{chapter.title}</h3><p>{chapter.character_count.toLocaleString()} characters · {chapter.word_count === undefined ? 'Runtime unavailable' : `Est. ${formatDuration(chapter.word_count / 150 * 60)}`}</p><button type="button" onClick={() => startChapter(chapter)}>Open in editor</button></div></article>)}</div>}
          ...
          {workspaceMode === 'review' && librarySection === 'assets' && (() => {
            const assets = workspaceAssetList().filter((asset) => {
              ...
              const matchesFilter = assetFilter === 'all' || (assetFilter === 'audio' && asset.type === 'Export') || (assetFilter === 'images' && asset.type === 'Cover Art') || (assetFilter === 'documents' && asset.type === 'Document') || (assetFilter === 'references' && asset.type === 'Reference');
            ...
            return <section className="audiobook-card audiobook-assets-panel">
              ...
              <div className="audiobook-filter-row">{([['all', 'All Assets'], ['audio', 'Audio'], ['images', 'Images'], ['documents', 'Documents'], ['references', 'References']] as [string, string][]).map(([key, label]) => <button type="button" key={key} className={assetFilter === key ? 'selected' : ''} onClick={() => setAssetFilter(key)}>{label}<span>{key === 'all' ? workspaceAssetList().length : workspaceAssetList().filter((asset) => key === 'audio' ? asset.type === 'Export' : key === 'images' ? asset.type === 'Cover Art' : key === 'documents' ? asset.type === 'Document' : asset.type === 'Reference').length}</span></button>)}</div>
          ...
          {workspaceMode === 'review' && librarySection === 'characters' && <section className="audiobook-card audiobook-characters-panel" id="audiobook-cast-main">
            ...
            <div className="audiobook-cast-grid">{project.speakers.map((speaker) => <article className="audiobook-cast-card" key={speaker.id}><div><p className="audiobook-avatar">{speaker.canonical_name.slice(0, 1).toUpperCase()}</p><h3>{speaker.canonical_name}</h3><small>{speaker.kind} · {speaker.casting ? 'Voice assigned' : 'Needs a voice'}</small></div>
              <label>Assigned voice<select value={speaker.casting?.voice_profile_id ?? ''} disabled={busy} aria-label={`Main voice for ${speaker.canonical_name}`} onChange={(event) => { const voice_profile_id = event.currentTarget.value; if (voice_profile_id) void action(() => omnixApiClient.post(`${base}/projects/${encodeURIComponent(project.id)}/speakers/${encodeURIComponent(speaker.id)}/casting`, { voice_profile_id }), `Voice assigned to ${speaker.canonical_name}.`); }}><option value="">Choose voice</option>{voicesQuery.data?.voices.map((voice) => <option key={voice.id} value={voice.id}>{voice.name}{voice.language ? ` · ${voice.language}` : ''}</option>)}</select></label>
