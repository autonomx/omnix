...
def test_send_chat_request_validates_supported_image_data_url():
    request = SendChatMessageRequest(content="Describe this", image_data_url=IMAGE_DATA_URL)

    assert request.image_data_url == IMAGE_DATA_URL

    with pytest.raises(ValidationError, match="PNG, JPEG, or WebP"):
        SendChatMessageRequest(content="Describe this", image_data_url="data:image/gif;base64,R0lGODlh")

    with pytest.raises(ValidationError, match="valid base64"):
        SendChatMessageRequest(content="Describe this", image_data_url="data:image/png;base64,not base64")
