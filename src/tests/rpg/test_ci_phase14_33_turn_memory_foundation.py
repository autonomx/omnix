...
def test_turn_memory_writes_recent_turn_and_dialogue_fact() -> None:
    updated_session, written = write_turn_memory(
        memory_session(), bran_result(player_input="Bran, my trail name is Red Fox."), player_input="Bran, my trail name is Red Fox."
    )
    ...
    assert written["facts"] == [{"type": "identity_alias", "subject": "player", "key": "trail_name", "value": "Red Fox"}]
...
def test_turn_memory_retrieves_private_bran_memory_for_bran_recall() -> None:
    session_after_fact, _ = write_turn_memory(
        memory_session(), bran_result(tick=1, player_input="Bran, my trail name is Red Fox."), player_input="Bran, my trail name is Red Fox."
    )
    session_after_filler, _ = write_turn_memory(
        session_after_fact, bran_result(tick=2, player_input="What food is on hand?"), player_input="What food is on hand?"
    )
