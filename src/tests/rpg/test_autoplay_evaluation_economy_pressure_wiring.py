...
def test_evaluation_uses_economy_pressure_summary():
    ...
    gate = evaluation["gates"]["economy_pressure_present"]
    assert gate["ok"] is True
    assert gate["value"]["event_count"] == 12
    assert gate["value"]["paid_count"] == 12
    assert "economy_pressure_present" not in evaluation["failed_gates"]