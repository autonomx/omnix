...
def test_panel_chrome_gate_is_ordered_before_runtime_manifest():
    workflow = WORKFLOW.read_text(encoding="utf-8")
    gate = "RPG CI Phase 8 panel chrome visibility gate"
    assert gate in workflow
    assert "test_ci_phase8_panel_chrome_visibility.py" in workflow
    assert workflow.index("RPG CI Phase 8 panel layout registry gate") < workflow.index(gate)
