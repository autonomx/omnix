...
def test_evaluation_uses_combat_lifecycle_summary():
    ...
    gate = evaluation["gates"]["combat_lifecycle_present"]
    assert gate["ok"] is True
    assert gate["value"]["encounter_count"] == 5
    assert gate["value"]["event_count"] == 5
    assert "combat_lifecycle_present" not in evaluation["failed_gates"]