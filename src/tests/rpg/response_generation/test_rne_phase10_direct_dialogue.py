...
def test_direct_dialogue_is_republished_as_ordered_canonical_blocks() -> None:
    ...
    assert result["npc"] == {"speaker": "npc:bran", "line": "The road is muddy, but passable."}
...
def test_runtime_installs_one_unified_direct_dialogue_publisher() -> None:
    session_init = (REPO_ROOT / "src/app/rpg/session/__init__.py").read_text(encoding="utf-8")
    gateway = (REPO_ROOT / "src/app/gateway/rpg_turn_pipeline.py").read_text(encoding="utf-8")
    assert "install_interactive_direct_dialogue_cutover" in session_init
    assert "install_interactive_direct_dialogue_cutover(interactive_first_call_runtime)" in gateway
