...
def test_generation_database_authentication_error_explains_that_postgres_is_reachable() -> None:
    with pytest.raises(HTTPException) as raised:
        _raise_generation_error(
            DatabaseUnavailableError("PostgreSQL operation failed", sqlstate="28P01"),
            operation="retry_failed",
    ...
    assert raised.value.status_code == 503
    assert raised.value.detail["error"] == "world_generation_database_authentication_failed"
...
def test_continue_generation_restores_a_partial_review_run_and_reuses_completed_topics(
    ...
    graph = CampaignTopicGraph(
        ...
        nodes=(
            CampaignTopicNode("realm", "Realm", "lore"),
            CampaignTopicNode("classes", "Classes", "classes", dependencies=("realm",)),
            CampaignTopicNode("factions", "Factions", "lore", dependencies=("classes",)),
        ),
    )
    settings = WorldTopicGenerationSettings(
        generator_version="original-generator",
    ...
    failed_run = {
        ...
        "settings": settings.as_dict(),
        "progress": {"failed_topic_ids": ["classes"]},
        "context": {"generation_context": {"genre": "fantasy"}},
    }
    ...
    monkeypatch.setattr("app.rpg.worlds.generation_retry._graph_from_payload", lambda payload: graph)
    monkeypatch.setattr("app.rpg.worlds.generation_retry._settings_from_payload", lambda payload: settings)
    monkeypatch.setattr(
        "app.rpg.worlds.generation_retry.resolve_generation_scope",
        lambda *args, **kwargs: (
            ("realm", "classes", "factions"),
            (),
            {"mode": "full", "resolved_topic_ids": ["realm", "classes", "factions"]},
        ),
    )
    monkeypatch.setattr(
        "app.rpg.worlds.generation_retry.start_world_generation",
        lambda **kwargs: captured.update(kwargs) or {"run_id": "run:continue", "status": "running"},
    )
    monkeypatch.setattr("app.rpg.worlds.generation_retry.kick_world_generation_worker", lambda **kwargs: True)
    monkeypatch.setattr("app.rpg.worlds.generation_retry.log_world_generation_event", lambda *args, **kwargs: {})
