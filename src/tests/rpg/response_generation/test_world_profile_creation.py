...
def test_known_genre_binds_profile_without_provider_job() -> None:
...
def test_llm_profile_proposal_compiles_with_core_and_setting_domains() -> None:
    ...
    assert {"setting_rules", "places", "groups", "actors", "pressures"}.issubset(
        domains
    )
...
def test_ready_profile_manifest_uses_pinned_cyberpunk_graph() -> None:
    plan = plan_world_profile_creation(
    ...
    assert {"networks", "augmentations"}.issubset(ids)
    assert {"spells", "races", "classes", "pantheon"}.isdisjoint(ids)
