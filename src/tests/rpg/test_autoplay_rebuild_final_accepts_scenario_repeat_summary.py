...
def test_rebuild_final_accepts_scenario_repeat_summary_keyword():
    summary = {
        "requested_turns": 100,
        "turns_executed": 100,
        "runtime_errors": [],
    ...
    rebuilt = _rebuild_final_100_turn_evaluation(
        ...
        turn_action_consistency_summary=summary["turn_action_consistency_summary"],
        scenario_progression_action_repeat_summary=summary[
            "scenario_progression_action_repeat_summary"
        ],
    )

    gate = rebuilt["hundred_turn_evaluation"]["gates"][
        "scenario_progression_repeats_bounded"
    ]

    assert gate["ok"] is True
    assert gate["value"]["repeat_warning_count"] == 1
    assert "scenario_progression_repeats_bounded" not in rebuilt[
