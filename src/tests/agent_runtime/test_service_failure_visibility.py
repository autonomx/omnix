...
def test_missing_validation_is_retried_before_quality_failure(monkeypatch) -> None:
    validation = ValidationSpec(
        id="browser-validation",
        kind="browser",
        description="Prove the requested rendered UI state.",
    )
    ...
    fingerprint = service_module.hashlib.sha256(
        "browser-validation".encode("utf-8")
    ).hexdigest()[:24]
    ...
    class Harness:
        ...
        def _set_quality_stage(self, *args, **kwargs):
            self.stage_calls.append((args, kwargs))
