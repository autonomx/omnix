...
Neither adapter reconstructs a historical screener later. Each provider/scanner candidate requires an observation timestamp, and field-level evidence timestamps later than the universe freeze are rejected. Prospective Finviz discovery is single-request and bounded to the first screener page (maximum 20 names); live pagination is forbidden because adjacent requests can observe different populations. Finviz archives retain the atomic-source provenance tag, source locator, and exact ordered source-symbol cohort so missing enrichment cannot silently erase the original discovery population. Legacy paginated Finviz archives fail the prospective-integrity gate.
...
Time-of-day RVOL compares current cumulative volume with prior sessions truncated at the same New York clock minute. Missing premarket bars/volume, TOD RVOL, or spread evidence is persisted as candidate data-quality state and produces `DATA_INCOMPLETE` rather than a synthetic low-liquidity rejection. Runtime 1-minute bars are bound to the frozen universe session date; a current session that lacks the 09:30 ET opening minute is not prospectively evaluated. Missing secondary evidence remains explicit rather than causing the candidate to disappear from the denominator.
