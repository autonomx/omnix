...
export function SettingsCategoryRail({ activeCategory, query, onQueryChange, onSelect }: {
  ...
  const categories = SETTINGS_CATEGORIES.filter((category) => !normalized || [category.label, category.description, ...(category.searchAliases ?? [])].join(' ').toLowerCase().includes(normalized));
  ...
  return (
    <aside className="settings-category-rail" aria-label="Settings categories">
      ...
      <label className="settings-rail-search">
        ...
        <input aria-label="Search settings categories" value={query} onChange={(event) => onQueryChange(event.currentTarget.value)} placeholder="Search settings…" />
      </label>
      <nav>
        {categories.map((category) => (
          ...
          >
            <span className="settings-category-icon" aria-hidden="true">{categoryGlyphs[category.id]}</span>
