...
export function OperationsSettings({ view = 'storage' }: { view?: OperationsSettingsView }) {
  ...
  return <div><h2>Jobs, Assets & Storage</h2><SettingsSection title="Output defaults" scope="global"><SettingsField label="Retention days"><input type="number" min="1" max="3650" value={storage.retentionDays} onChange={(event) => dispatch({ type: 'update', path: 'storage.retentionDays', value: Number(event.currentTarget.value) })} /></SettingsField></SettingsSection></div>;
}
