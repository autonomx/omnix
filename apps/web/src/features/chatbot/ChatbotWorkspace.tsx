...
function AssistantWorkspaceView({ activeView, chatProviders, enabledToolCount, modelLabel, onShowTools, onStartLiveCall, providerLabel, runtimeConfig, toolExecutionRows }: { activeView: Exclude<AssistantView, 'chats'>; chatProviders: ReturnType<typeof chatCapableProviders>; enabledToolCount: number; modelLabel: string; onShowTools: () => void; onStartLiveCall: () => void; providerLabel: string; runtimeConfig: AssistantWorkspaceRuntimeConfig; toolExecutionRows: number }) {
  ...
  if (activeView === 'tools') return <section className="assistant-view-panel" aria-label="Tools view"><p className="eyebrow">Omnix Assistant</p><h2>Tools</h2><p>Configure assistant-only tool access and monitor execution. Tool calls remain approval-gated and auditable.</p><div className="assistant-tool-config-list"><ToolAccessRow name="Gmail" description="Review connection, scopes, and approval policy for Gmail-assisted workflows." status="Review access" /><ToolAccessRow name="Google Calendar" description="Read availability and manage scheduling workflows with user approval." status="Configured" /><ToolAccessRow name="Google Contacts" description="Resolve saved contacts for assistant workflows." status="Configured" /><ToolAccessRow name="GitHub" description="Review PRs, inspect Actions, and create implementation branches." status="Configured" /><ToolAccessRow name="Web search" description="Use public web context only when freshness or verification is required." status="Approval required" /></div><button type="button" onClick={onShowTools}>Show execution panel</button><p className="assistant-view-note">{enabledToolCount} tools active · {toolExecutionRows} replayed execution rows</p></section>;
...
function ToolAccessRow({ name, description, status }: { name: string; description: string; status: string }) { return <article><div><h3>{name}</h3><p>{description}</p></div><strong>{status}</strong></article>; }
