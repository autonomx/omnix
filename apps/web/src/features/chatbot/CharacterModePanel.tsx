...
export function CharacterModePanel({ sessionId }: { sessionId: string }) {
  ...
  const mutation = useMutation({
    mutationFn: (mode: 'system' | 'character') => characterClient.setSession(sessionId, {
      interaction_mode: mode,
      character_id: mode === 'character' ? selectedCharacterId : null,
      voice_asset_id: mode === 'character'
        ? characters.find((item) => item.id === selectedCharacterId)?.default_voice_asset_id ?? null
        : null,
    }),
    ...
    <article className="character-mode-card" aria-label="Character Mode settings">
      ...
      <div className="character-mode-controls">
        ...
        <div className="character-mode-actions">
          <button
            type="button"
            disabled={!selectedCharacterId || mutation.isPending || characterMode}
