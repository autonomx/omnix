...
export function VoiceWorkspace({ module }: { module: OmnixModuleDefinition }) {
  ...
  const {
    ...
    watch,
    formState: { errors },
  } = useForm<VoiceFormValues>({ defaultValues: { text: '', speaker: '', voiceId: '', providerId: '' } });
      ...
      <div className="voice-studio-shell">
        ...
        <div className="voice-dashboard-grid">
          <section className="voice-panel">
            <Group justify="space-between" align="start">
              <div>
                ...
                <Text size="sm">Build single voice or multi-speaker narration jobs.</Text>
              </div>
            ...
            <form className="voice-studio-field-grid" onSubmit={handleSubmit((values) => createJobMutation.mutate(values))}>
              ...
              <div className="voice-studio-field-wide voice-studio-field">
                Voice Source
                <div className="voice-source-tabs" aria-label="Voice source">
                  <button type="button">System Voices</button>
                  <button type="button" className="active">Voice Clones (Local)</button>
                </div>
              </div>
              ...
              <label>
                ...
                <select {...register('voiceId')}>
                  ...
                  {profileAssets.map((asset) => (
                    <option key={asset.id} value={asset.storage_path}>{voiceAssetName(asset)}</option>
                  ))}
                </select>
              </label>
              <label>
                Speaker / Preset
                <input {...register('speaker')} placeholder="Narrator (Neutral)" />
              </label>
              <label>
                Voice ID (optional)
                <input placeholder="e.g. narrator_v1" readOnly value={profileAssets[0]?.id ?? ''} />
              </label>
              <label>
                Style Prompt (optional)
                <input placeholder="authoritative, calm, cinematic" readOnly value="" />
              </label>

              <div className="voice-studio-field-wide voice-slider-grid">
                {Object.entries(DEFAULT_OUTPUT_SETTINGS).map(([name, value]) => (
                  <label className="voice-slider-card" key={name}>
                    <span>{formatSettingName(name)}</span>
              ...
              <label className="voice-studio-field-wide">
                Text to synthesize
                <textarea rows={7} aria-invalid={Boolean(errors.text)} placeholder="Dave: hello there&#10;Bob: how do you do&#10;Marry: I'm doing fine" {...register('text', { required: true })} />
              </label>

              <div className="voice-primary-actions voice-studio-field-wide">
                <Button type="button" variant="light">▶ Preview</Button>
                <Button className="voice-action-cyan" type="submit" disabled={createJobMutation.isPending} loading={createJobMutation.isPending}>☰ Queue Synthesis</Button>
                <Button type="submit" variant="light" disabled={createJobMutation.isPending}>▷ Generate & Play</Button>
                <Button type="button" variant="subtle">■ Stop</Button>
              </div>
            </form>
          ...
          <section className="voice-panel">
            <Group justify="space-between" align="start">
              <div>
                <Title order={4}>▣ Voice Clone Library</Title>
                <Text size="sm">Local cloned voices from resources/voice_clones</Text>
              </div>
              <OmnixStatusPill>{profileAssets.length} voices</OmnixStatusPill>
            </Group>
            {profileAssets.length ? (
              <div className="voice-clone-grid">
                {profileAssets.map((asset, index) => <VoiceCloneCard key={asset.id} asset={asset} active={index === 0} />)}
              </div>
            ) : (
              <div className="platform-empty" role="status">No voice profiles indexed.</div>
            )}
          ...
          <section className="voice-panel">
            <Group justify="space-between" align="start">
              <div>
                ...
                <Text size="sm">Active queue, recent jobs, and latest preview.</Text>
              </div>
              <Button size="xs" variant="subtle">Clear Completed</Button>
            </Group>
            <div className="voice-queue-list">
              <div className="voice-queue-row voice-table-heading"><span>Job</span><span>Status</span><span>Provider</span><span>Time</span></div>
              {(voiceJobs.length ? voiceJobs : []).slice(0, 5).map((job) => (
                <div className="voice-queue-row" key={job.id}>
                  <span><b>{job.type}</b><small>{job.module}</small></span>
                  <span><OmnixStatusPill>{job.status}</OmnixStatusPill><Progress value={progressPercent(job.progress)} /></span>
                  <span>Default TTS</span>
                  <span>{job.progress?.message ?? '—'}</span>
                </div>
              ))}
              {!voiceJobs.length ? <div className="platform-empty" role="status">No voice jobs queued.</div> : null}
            </div>
            <div className="voice-player-panel">
              <Group justify="space-between"><Text size="sm">Latest Preview · {latestResultAsset ? voiceAssetName(latestResultAsset) : 'Narrator (Neutral)'}</Text><Text size="sm">00:06 / 00:12</Text></Group>
              <Waveform />
              <Group gap="xs"><Button size="xs" variant="light">Ⅱ</Button><Button size="xs" variant="subtle">↶</Button><Button size="xs" variant="subtle">↷</Button><Button size="xs" variant="subtle">Export</Button><Button size="xs" variant="subtle">Keep</Button></Group>
            </div>
          </section>
          ...
          <section className="voice-panel voice-panel-wide">
            ...
            {generatedAudioAssets.length ? (
              <div className="voice-asset-table">
                <div className="voice-asset-row voice-table-heading"><span></span><span>File Name</span><span>Voice</span><span>Provider</span><span>Duration</span><span>Actions</span></div>
                {generatedAudioAssets.slice(0, 6).map((asset) => <AudioAssetRow key={asset.id} asset={asset} />)}
              </div>
            ) : (
              <div className="platform-empty" role="status">No audio assets indexed.</div>
            )}
          </section>
          ...
          <section className="voice-panel">
            ...
            <div className="voice-diagnostics-grid">
              <DiagnosticCard title="TTS Provider" status={ttsProviders[0]?.label ?? 'Default TTS'} tone="online" detail="Latency 128 ms" />
              <DiagnosticCard title="Queue Worker" status="Online" tone="online" detail={`${voiceJobs.length} jobs`} />
              <DiagnosticCard title="GPU (CUDA)" status="Idle" tone="idle" detail="Memory available" />
              <DiagnosticCard title="CPU" status="Ready" tone="idle" detail="Resources normal" />
            </div>
            <div className="voice-diagnostic-list">
              {['TTS provider health check passed', 'Queue worker heartbeat OK', 'GPU status OK', 'System resources within normal limits'].map((message) => (
                <div className="voice-diagnostic-row" key={message}><span>✓</span><span>{message}</span><span>now</span></div>
              ))}
            </div>
          </section>
        </div>
...
function VoiceCloneCard({ asset, active }: { asset: VoiceAsset; active: boolean }) {
  return (
    <article className={`voice-clone-card${active ? ' active' : ''}`}>
      <Group justify="space-between"><Group gap="xs"><span className="voice-avatar">◉</span><strong>{voiceAssetName(asset)}</strong></Group><OmnixStatusPill>✓</OmnixStatusPill></Group>
      <Text size="sm"><span className="voice-status-dot" /> Ready</Text>
      <div className="voice-meta-list"><span>Profile</span><b>{asset.id}</b><span>Sample Rate</span><b>48 kHz</b><span>Source</span><b>{asset.storage_path}</b></div>
      <div className="voice-card-actions"><Button size="xs" variant="subtle">▷ Preview</Button><Button size="xs" variant="light">Use</Button></div>
    </article>
  );
}
...
function AudioAssetRow({ asset }: { asset: VoiceAsset }) {
  return (
    <div className="voice-asset-row">
      <span>▶</span><span><b>{voiceAssetName(asset)}</b><small>{asset.storage_path}</small></span><span>Narrator</span><span>Default TTS</span><span>00:12</span><span><Button size="xs" variant="subtle">▷</Button><Button size="xs" variant="subtle">⇩</Button></span>
    </div>
  );
}
