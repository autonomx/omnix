...
const asset = {
  ...
  metadata: { title: 'Night harbor', width: 1024, height: 768, provider_key: 'flux_klein' },
} as ImageAsset;
...
describe('ImageLatestResult', () => {
  it('announces completed results and exposes asset-id actions', () => {
    ...
    expect(screen.getByRole('img', { name: 'Night harbor' })).toHaveAttribute('src', '/api/assets/image%3Anight/file?preview=true');
    expect(screen.getByRole('link', { name: 'Open Night harbor in a new tab' })).toHaveAttribute('href', '/api/assets/image%3Anight/file?preview=true');
    expect(screen.getByRole('link', { name: 'Download Night harbor' })).toHaveAttribute('href', '/api/assets/image%3Anight/file?download=true');
  ...
  it('opens and closes an enlarged image preview when the result image is clicked', () => {
    ...
    expect(within(dialog).getByTestId('image-preview-loader')).toHaveAttribute('src', '/api/assets/image%3Anight/file?preview=true');
