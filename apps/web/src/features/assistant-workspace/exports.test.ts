...
describe('assistant workspace public exports', () => {
  it('exposes helper modules added after the foundation phases', () => {
    ...
    expect(createWorkspaceProjectTree({ workspace: { id: 'workspace-1', name: 'W' }, projects: [], conversations: [] }).workspace.id).toBe('workspace-1');
    ...
    expect(createContextPanelSummary({
      ...
      omittedSources: [],
      estimatedTokens: 0,
      budget: { maxTokens: 10, reservedResponseTokens: 2, sourceTokenLimit: 8 },
    }).maxTokens).toBe(10);
  });
});
