...
export type TradingChartState = { chartId: string; instrumentId: string; interval: string; chartType: TradingChartType; indicators: CoreIndicatorInstance[] };
export type TradingLinkState = { instrument: boolean; interval: boolean; crosshair: boolean; visibleRange: boolean };
...
const indicators = (): CoreIndicatorInstance[] => [
  { id: 'sma', period: 20, enabled: true }, { id: 'ema', period: 20, enabled: false }, { id: 'rsi', period: 14, enabled: true },
];

export const useTradingStore = create<TradingWorkspaceState>((set) => ({
  layout: 'one', activeChartId: 'chart-1', drawingTool: 'cursor',
  charts: [
  ...
  setLayout: (layout) => set({ layout }), setActiveChart: (activeChartId) => set({ activeChartId }), setDrawingTool: (drawingTool) => set({ drawingTool }),
  updateChart: (chartId, patch) => set((state) => {
    ...
    return { charts: state.charts.map((chart) => chart.chartId === chartId ? { ...chart, ...patch } : { ...chart, ...(linkedInstrument ? { instrumentId: patch.instrumentId } : {}), ...(linkedInterval ? { interval: patch.interval } : {}) }) };
  }),
  toggleIndicator: (chartId, id, period) => set((state) => ({ charts: state.charts.map((chart) => chart.chartId !== chartId ? chart : { ...chart, indicators: chart.indicators.map((indicator) => indicator.id !== id ? indicator : { ...indicator, enabled: !indicator.enabled, period: period ?? indicator.period }) }) })),
