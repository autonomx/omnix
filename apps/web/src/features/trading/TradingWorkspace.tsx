  ...
  return (
    <main className={`trading-workspace${focusMode ? ' trading-focus-mode' : ''}`} aria-labelledby="trading-title">
      ...
      <section className="trading-toolbar" aria-label="Trading toolbar">
        ...
        <label>
          Instrument
          <select aria-label="Trading instrument" value={activeChart.instrumentId} onChange={(event) => updateChart(activeChartId, { instrumentId: event.target.value })}>
            {(instruments.data ?? []).map((instrument) => <option key={instrument.instrument_id} value={instrument.instrument_id}>{instrument.display_symbol} · {instrument.venue}</option>)}
          </select>
        </label>
        <label>
          Timeframe
          <select aria-label="Trading timeframe" value={activeChart.interval} onChange={(event) => updateChart(activeChartId, { interval: event.target.value })}>
        ...
        <label>
          ...
          <select aria-label="Trading chart type" value={activeChart.chartType} onChange={(event) => updateChart(activeChartId, { chartType: event.target.value as TradingChartType })}>
            <option value="candlestick">Candlestick</option><option value="line">Line</option>
          </select>
        </label>
      ...
      <section className="trading-link-toolbar" aria-label="Chart link controls">
        {(Object.keys(links) as Array<keyof typeof links>).map((key) => (
          <label key={key}><input type="checkbox" checked={links[key]} onChange={(event) => setLink(key, event.target.checked)} />Link {key === 'visibleRange' ? 'visible range' : key}</label>
        ))}
      </section>

      <div className="trading-body">
        <aside className="trading-tools" aria-label="Chart drawing tools">
          {['Cursor', 'Crosshair', 'Trend line', 'Horizontal line', 'Measure'].map((tool) => <button key={tool} type="button" title={tool} aria-label={tool}>{tool.slice(0, 2)}</button>)}
        </aside>
        ...
        <aside className="trading-side-panel" aria-label="Trading side panel">
          <nav aria-label="Trading panel tabs"><button type="button" className="active">Watchlist</button><button type="button">Indicators</button><button type="button">Data</button><button type="button">Layout</button></nav>
