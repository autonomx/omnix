    ...
    <article className={`trading-chart-panel${active ? ' active' : ''}`} data-chart-id={chartId} data-stream-status={streamStatus} onPointerDown={onActivate} aria-label={`${chartId}${active ? ', active chart' : ''}`}>
      <header>
        ...
        <div className="trading-chart-provenance">
          <span>{chartQuery.data?.binding.provider ?? 'binance'}</span><span>{provenance?.freshness_mode ?? 'loading'}</span><span className={`stream-${streamStatus}`}>{streamStatus}</span><span>{drawings.status}</span>
        </div>
      </header>
