...
export function TradingChartPanel({
  ...
  const chartQuery = useQuery({ queryKey: ['trading', 'bars', instrumentId, interval], queryFn: () => tradingApi.bars(instrumentId, interval, 1_000), enabled: Boolean(instrumentId), staleTime: 15_000 });
  ...
  useEffect(() => {
    ...
    return () => { unregister(); next.destroy(); adapterRef.current = null; setAdapter(null); };
  ...
  useEffect(() => { adapterRef.current?.setChartType(chartType, barsRef.current); }, [chartType]);
  useEffect(() => { adapterRef.current?.setIndicators(barsRef.current, indicators); }, [indicators]);

  useEffect(() => {
    if (!instrumentId) return;
    setStreamError(null);
    return tradingStreamHub.subscribe(chartId, instrumentId, interval, (message) => {
      if (message.type === 'error') { setStreamError(message.message); return; }
      ...
      if (adapterRef.current?.updateBar(bar)) {
        ...
        if (index >= 0) barsRef.current[index] = bar; else barsRef.current = [...barsRef.current, bar];
        adapterRef.current.setIndicators(barsRef.current, indicators);
      }
    }, (status) => { setStreamStatus(status); if (status === 'closed' || status === 'error') void chartQuery.refetch(); });
  }, [chartId, instrumentId, interval, indicators]);

  const provenance = chartQuery.data?.provenance;
  return (
    <article className={`trading-chart-panel${active ? ' active' : ''}`} data-chart-id={chartId} data-stream-status={streamStatus} onPointerDown={onActivate} aria-label={`${chartId}${active ? ', active chart' : ''}`}>
      <header>
        <div><strong>{chartQuery.data?.instrument.display_symbol ?? instrumentId}</strong><span>{interval} · {chartType}</span><span>{indicators.filter((item) => item.enabled).map((item) => item.id.toUpperCase()).join(' · ')}</span></div>
        <div className="trading-chart-provenance"><span>{chartQuery.data?.binding.provider ?? 'binance'}</span><span>{provenance?.freshness_mode ?? 'loading'}</span><span className={`stream-${streamStatus}`}>{streamStatus}</span><span>{drawings.status}</span></div>
        {active ? <div className="trading-drawing-actions"><button type="button" onClick={(event) => { event.stopPropagation(); drawings.undo(); }} aria-label="Undo drawing">↶</button><button type="button" onClick={(event) => { event.stopPropagation(); drawings.redo(); }} aria-label="Redo drawing">↷</button><button type="button" onClick={(event) => { event.stopPropagation(); drawings.removeSelected(); }} aria-label="Delete selected drawing">×</button></div> : null}
      </header>
      <div className="trading-chart-stage">
        <div ref={hostRef} className="trading-chart-canvas" aria-label={`${instrumentId} ${interval} chart`} />
        <TradingDrawingOverlay adapter={adapter} instrumentId={instrumentId} tool={active ? drawingTool : 'cursor'} drawings={drawings.state.drawings} selectedId={drawings.state.selectedId} onAdd={drawings.add} onSelect={drawings.select} onMovePoint={drawings.movePoint} />
      </div>
      ...
      <footer><span>{provenance?.cached ? 'Cached dataset' : 'Provider dataset'}</span><span>{provenance?.as_of ? `As of ${new Date(provenance.as_of).toLocaleString()}` : ''}</span><span>{provenance?.dataset_fingerprint ? provenance.dataset_fingerprint.slice(0, 10) : ''}</span></footer>
    </article>
  );
}
