...
function voiceForSpeaker(speakerName: string, speakers: SpeakerDraft[], voiceOptions: Array<{ id: string; label: string }>) { return speakers.find((speaker) => speaker.name === speakerName)?.voice || voiceOptions[0]?.id || null; }
...
export function PodcastWorkspace({ module }: { module: OmnixModuleDefinition }) {
  ...
  function queueLiveOutput(output: PlayablePodcastOutput) {
    setLiveOutputs((current) => { const next = [...current, output]; liveOutputsRef.current = next; return next; });
    const currentKey = selectedOutputKeyRef.current;
    if (!currentKey || currentKey.startsWith('__')) selectOutputKey(output.key);
  }
  ...
  const createJobMutation = useMutation({
    mutationFn: async () => {
      ...
      setDirectorNote('Script is ready. Live preview audio is queued as a playlist through the same Voice Studio TTS job path. New chunks will not interrupt the chunk currently playing.');
      ...
      for (const segment of segments) {
        ...
        if (output) {
          ...
          setActionMessage(`Queued live preview ${segment.index + 1}/${segments.length}: ${segment.speaker}`);
          debug('live turn queued', { index: segment.index, speaker: segment.speaker, job: completedSegmentJob.id });
      ...
      setDirectorNote('Live preview chunks are queued. Rendering the single final podcast output for seeking, replay, and download.');
      setActionMessage('Live preview queued. Rendering final podcast audio...');
    ...
    onMutate: () => { liveOutputsRef.current = []; setLiveOutputs([]); setTranscript([]); setLiveAutoplay(false); selectOutputKey('__script__'); setDirectorNote('Generating a real speaker-tagged script with the configured chat provider.'); setActionMessage('Requesting podcast script from LLM...'); },
    onSuccess: async (job) => { const output = extractPlayableOutputs([job])[0]; const currentKey = selectedOutputKeyRef.current; if (output && (!currentKey || currentKey.startsWith('__') || !currentKey.startsWith('live:'))) selectOutputKey(output.key); if (isFailed(job.status)) { setDirectorNote(jobErrorMessage(job)); setActionMessage(jobErrorMessage(job)); } else { setDirectorNote(output ? 'Final podcast audio is ready. The live preview playlist will continue without interruption; select the final render when you want the stitched output.' : 'Podcast render completed but no playable audio output was attached.'); setActionMessage(output ? `Podcast audio ready: ${job.id}` : `Podcast production completed without playable audio: ${job.id}`); } await Promise.all([queryClient.invalidateQueries({ queryKey: ['platform', 'jobs'] }), queryClient.invalidateQueries({ queryKey: ['platform', 'assets'] })]); },
  ...
  const transcriptRows = transcript.length ? transcript : persistedTranscript.length ? persistedTranscript : transcriptRowsFromJob(connectedJob);
  ...
  const activeStage = createJobMutation.isPending ? (selectedOutputKey === '__script__' ? 1 : selectedOutputKey === '__streaming__' ? 2 : 4) : connectedJob ? (firstIncomplete >= 0 ? firstIncomplete : (connectedJob.stages ?? []).length - 1) : -1;
  ...
  useEffect(() => { const audio = audioRef.current; if (!audio || !currentOutput) return; audio.playbackRate = Number.parseFloat(playbackRate) || 1; if (!liveAutoplay) return; const timer = window.setTimeout(() => { audio.play().catch(() => setActionMessage('Audio is ready. Press play if the browser blocked autoplay.')); }, 50); return () => window.clearTimeout(timer); }, [currentOutput?.key, liveAutoplay, playbackRate]);
  ...
  function resetPodcast() { setTitle(''); setBrief(''); setDuration('5 min'); setTranscript([]); liveOutputsRef.current = []; setLiveOutputs([]); setDirectorNote('New podcast request cleared. Add a title and brief, then generate.'); selectOutputKey('__new__'); setActionMessage('New podcast ready.'); }
  ...
  function onAudioEnded() { const currentKey = currentOutput?.key ?? selectedOutputKeyRef.current; const liveList = liveOutputsRef.current; const liveIndex = liveList.findIndex((output) => output.key === currentKey); if (liveIndex >= 0 && liveList[liveIndex + 1]) { selectOutputKey(liveList[liveIndex + 1].key); return; } const finalOutput = extractPlayableOutputs([createJobMutation.data, ...podcastJobs]).find((output) => !output.live); if (finalOutput) { selectOutputKey(finalOutput.key); return; } if (createJobMutation.isPending) setActionMessage('Waiting for the next live preview segment...'); }

  return (
    <WorkspacePanel className="podcast-workspace-panel">
      <div className="podcast-studio-shell">
        <header className="podcast-studio-header"><div><p className="eyebrow">Conversation engine</p><h2 id="module-title">{module.label}</h2><p>Create a real LLM-generated podcast script, queue live preview turns through Voice Studio TTS, then keep one final stitched render for playback and download.</p></div><code>/podcast-renderer</code></header>
