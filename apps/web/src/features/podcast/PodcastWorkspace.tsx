...
const debugPrefix = '[PODCAST][voice-studio-stream]';
...
function extractPlayableOutputs(jobs: Array<JobRecord | undefined>, liveOutputs: PlayablePodcastOutput[] = []): PlayablePodcastOutput[] {
  ...
  for (const job of jobs) {
    const refs = (job?.output_refs ?? []) as Array<{ data_url?: unknown; audio_url?: unknown; duration?: unknown; asset_id?: unknown; title?: unknown; mime_type?: unknown }>;
...
function parseSegmentsFromLlm(raw: string): Array<{ index: number; speaker: string; text: string }> {
  ...
  if (jsonMatch) {
    try { const parsed = JSON.parse(jsonMatch[0]); const rows = normalizeSegments(parsed); if (rows.length) return rows; } catch (error) { console.warn(debugPrefix, 'LLM JSON parse failed', error); }
  }
...
async function generateLlmPodcastSegments(args: { title: string; brief: string; audience: string; duration: string; format: PodcastFormat; speakers: SpeakerDraft[]; tone: string; language: string }) {
  ...
  const prompt = `Generate a real, speaker-tagged podcast script. Return JSON only with this shape: {"segments":[{"speaker":"Host","text":"spoken line"}]}.
Topic: ${args.title}
Brief: ${args.brief}
...
${speakerList}
Make the dialogue specific to the topic. Avoid stage directions, markdown, bullets, and narrator notes.`;
  try {
    ...
    const rows = parseSegmentsFromLlm(text);
    if (rows.length >= Math.min(2, args.speakers.length)) {
      debug('LLM script received', { rows: rows.length });
      return rows.map((segment, index) => ({ ...segment, index }));
    }
    console.warn(debugPrefix, 'LLM script response did not contain usable segments; falling back to local planner.', text.slice(0, 240));
  } catch (error) {
...
function voiceForSpeaker(speakerName: string, speakers: SpeakerDraft[], voiceOptions: Array<{ id: string; label: string }>) {
  return speakers.find((speaker) => speaker.name === speakerName)?.voice || voiceOptions[0]?.id || null;
}
function buildSingleSpeakerJobPayload(args: { title: string; language: string; segment: { index: number; speaker: string; text: string }; speakers: SpeakerDraft[]; voiceOptions: Array<{ id: string; label: string }> }) {
  const voiceId = voiceForSpeaker(args.segment.speaker, args.speakers, args.voiceOptions);
  return { title: `${args.title} - ${args.segment.speaker} ${args.segment.index + 1}`, text: args.segment.text, provider_id: null, speaker: args.segment.speaker, voice_id: voiceId, language: args.language, script_mode: 'single_speaker', script_speakers: [{ name: args.segment.speaker, count: 1 }], script_segments: [args.segment], character_voice_assignments: [{ speaker: args.segment.speaker, voice_id: voiceId, style: firstTag(args.speakers.find((speaker) => speaker.name === args.segment.speaker)?.speakingStyle || ''), line_count: 1 }], output_settings: outputSettings, audio_effects: audioEffects, save_output: true, renderer: 'podcast-live-preview' };
}
function buildPodcastJobPayload(args: any) {
  ...
  return { title: args.title, brief: args.brief, format: args.format, audience: args.audience, duration_minutes: durationMinutes(args.duration), target_duration_seconds: targetSeconds, target_word_count: targetWords, tone: args.tone, language: args.language, generation_style: args.generationStyle, review_policy: args.reviewPolicy, renderer: 'podcast', text: args.segments.map((segment) => `${segment.speaker}: ${segment.text}`).join('\n'), provider_id: null, speaker: args.speakers[0]?.name || 'Host', voice_id: firstVoice, script_mode: args.segments.length > 1 ? 'multi_speaker' : 'single_speaker', script_speakers: Object.entries(counts).map(([name, count]) => ({ name, count })), script_segments: args.segments, character_voice_assignments: args.speakers.map((speaker, index) => ({ speaker: speaker.name, voice_id: speaker.voice || args.voiceOptions[index % Math.max(args.voiceOptions.length, 1)]?.id || null, style: firstTag(speaker.speakingStyle || speaker.personality), line_count: counts[speaker.name] ?? 0 })), output_settings: outputSettings, audio_effects: audioEffects, save_output: true, speakers: args.speakers.map((speaker) => ({ id: speaker.id, name: speaker.name, role: speaker.role, identity: speaker.identity, beliefs: splitTags(speaker.beliefs), personality: splitTags(speaker.personality), speakingStyle: splitTags(speaker.speakingStyle), defaultGoal: speaker.goal, speakerInstructions: speaker.instructions, voiceMapping: { speakerId: speaker.id, voiceId: speaker.voice, voiceDisplayName: args.voiceOptions.find((voice) => voice.id === speaker.voice)?.label || speaker.voice, previewAvailable: Boolean(speaker.voice) } })), relationships: mockPodcastRelationships, relationship_overrides: args.relationships, constraints: { maxDurationSeconds: targetSeconds, targetDurationSeconds: targetSeconds, targetWordCount: targetWords, maxSpeakerTurnSeconds: Number.parseInt(args.maxTurnSeconds, 10) || 45, citationRequired: args.citationRequired === 'On', familyFriendly: args.familyFriendly === 'On', readingLevel: args.readingLevel, avoidTopics: splitTags(args.avoidTopics), requiredTopics: ['practical examples', 'risks', 'future outlook'], disallowedClaims: [], tone: args.tone, audience: args.audience, language: args.language } };
}
...
export function PodcastWorkspace({ module }: { module: OmnixModuleDefinition }) {
  ...
  const createJobMutation = useMutation({
    mutationFn: async () => {
      ...
      setDirectorNote('Script is ready. Streaming preview audio through the same Voice Studio TTS job path, then rendering one final stitched podcast output.');
      setActionMessage(`Script ready: ${segments.length} speaking turns. Starting live audio preview...`);
      debug('script ready', { segments: segments.length });

      const liveGenerated: PlayablePodcastOutput[] = [];
      for (const segment of segments) {
        const segmentJob = await omnixApiClient.createJob({ module: 'podcast', type: 'tts.synthesize', resource_class: 'gpu:tts', priority: 1, input_payload: buildSingleSpeakerJobPayload({ title, language, segment, speakers, voiceOptions }), stages: [{ id: 'prepare_turn', label: 'Prepare speaking turn', resource_class: 'cpu', status: 'queued' }, { id: 'voice_turn', label: `Voice Turn: ${segment.speaker}`, resource_class: 'gpu:tts', status: 'queued' }] }, { timeoutMs: 120000, timeoutMessage: 'Podcast live preview turn is still running.' });
        ...
        if (output) {
          const liveOutput = { ...output, key: `live:${segment.index}:${output.key}`, title: `${segment.speaker}: ${output.title}`, live: true };
          liveGenerated.push(liveOutput);
          setLiveOutputs((current) => [...current, liveOutput]);
          setSelectedOutputKey(liveOutput.key);
          setActionMessage(`Live preview playing ${segment.index + 1}/${segments.length}: ${segment.speaker}`);
          debug('live turn ready', { index: segment.index, speaker: segment.speaker, job: completedSegmentJob.id });
        } else if (isFailed(completedSegmentJob.status)) {
          console.warn(debugPrefix, 'live preview turn failed', completedSegmentJob);
          setActionMessage(jobErrorMessage(completedSegmentJob) || `Live preview turn failed: ${segment.speaker}`);
        }
      }

      setDirectorNote('Live preview finished. Rendering the single final podcast output for seeking, replay, and download.');
      setActionMessage('Live preview complete. Rendering final podcast audio...');
      const finalJob = await omnixApiClient.createJob({ module: 'podcast', type: 'tts.multi_speaker_synthesize', resource_class: 'gpu:tts', priority: 0, input_payload: buildPodcastJobPayload({ title, brief, format, audience, duration, tone, language, generationStyle, reviewPolicy, speakers, voiceOptions, segments, citationRequired, familyFriendly, readingLevel, maxTurnSeconds, avoidTopics, relationships }), stages: podcastStages() }, { timeoutMs: 180000, timeoutMessage: 'Final podcast render is still running.' });
      const completedFinalJob = await waitForPlayableJob(finalJob, 180000);
      const finalOutput = extractPlayableOutputs([completedFinalJob])[0];
      if (finalOutput) setSelectedOutputKey(finalOutput.key);
    ...
    onMutate: () => { setLiveOutputs([]); setTranscript([]); setLiveAutoplay(false); setSelectedOutputKey('__script__'); setDirectorNote('Generating a real speaker-tagged script with the configured chat provider. Check the browser console for podcast debug events.'); setActionMessage('Requesting podcast script from LLM...'); },
    onSuccess: async (job) => { const output = extractPlayableOutputs([job])[0]; if (output) setSelectedOutputKey(output.key); if (isFailed(job.status)) { setDirectorNote(jobErrorMessage(job)); setActionMessage(jobErrorMessage(job)); } else { setDirectorNote(output ? 'Podcast audio is ready. Live preview used the Voice Studio TTS path; this is the final stitched output.' : 'Podcast render completed but no playable audio output was attached.'); setActionMessage(output ? `Podcast audio ready: ${job.id}` : `Podcast production completed without playable audio: ${job.id}`); } await Promise.all([queryClient.invalidateQueries({ queryKey: ['platform', 'jobs'] }), queryClient.invalidateQueries({ queryKey: ['platform', 'assets'] })]); },
  ...
  const playableOutputs = useMemo(() => extractPlayableOutputs([createJobMutation.data, ...podcastJobs], liveOutputs), [createJobMutation.data, podcastJobs, liveOutputs]);
  ...
  const liveStatus = createJobMutation.isPending ? (selectedOutputKey === '__script__' ? 'SCRIPTING' : selectedOutputKey === '__streaming__' ? 'STREAMING' : 'RENDERING') : connectedJob ? String(connectedJob.status).toUpperCase() : 'IDLE';
  ...
  const activeStage = createJobMutation.isPending ? (selectedOutputKey === '__script__' ? 1 : selectedOutputKey === '__streaming__' ? 2 : 4) : connectedJob ? (firstIncomplete >= 0 ? firstIncomplete : jobStages.length - 1) : -1;
  const stages = createJobMutation.isPending ? plannedStages.map((stage, index) => ({ ...stage, state: index === activeStage ? 'active' : index < activeStage ? 'done' : 'pending' })) : jobStages.length ? jobStages.map((stage, index) => ({ id: stage.id, label: stage.label, state: stageState(stage.status, index, activeStage) })) : plannedStages.map((stage) => ({ ...stage, state: 'pending' }));
  ...
  useEffect(() => { if (!selectedOutputKey && playableOutputs.length > 0) setSelectedOutputKey(playableOutputs[0].key); if ((selectedOutputKey === '__streaming__' || selectedOutputKey === '__script__') && playableOutputs.length) setSelectedOutputKey(playableOutputs[playableOutputs.length - 1].key); }, [playableOutputs, selectedOutputKey]);
  useEffect(() => { const audio = audioRef.current; if (!audio || !currentOutput) return; audio.playbackRate = Number.parseFloat(playbackRate) || 1; if (!liveAutoplay) return; const timer = window.setTimeout(() => { audio.play().catch((error) => { console.warn(debugPrefix, 'autoplay blocked or failed', error); setActionMessage('Audio is ready. Press play if the browser blocked autoplay.'); }); }, 50); return () => window.clearTimeout(timer); }, [currentOutput?.key, liveAutoplay, playbackRate]);
  ...
  function resetPodcast() { setTitle(''); setBrief(''); setAudience('Software Engineers'); setDuration('5 min'); setTone('Professional'); setLanguage('English (US)'); setFormat('debate'); setGenerationStyle('automatic'); setManualReviewStops([]); setTranscript([]); setLiveOutputs([]); setDirectorNote('New podcast request cleared. Add a title and brief, then generate.'); setSelectedOutputKey('__new__'); setActionMessage('New podcast ready.'); }
  ...
  function onAudioEnded() { const currentIndex = playableOutputs.findIndex((output) => output.key === currentOutput?.key); const next = currentIndex >= 0 ? playableOutputs[currentIndex + 1] : null; if (next && next.live) { setSelectedOutputKey(next.key); return; } if (createJobMutation.isPending) setActionMessage('Waiting for the next live preview segment...'); }

  return (
    <WorkspacePanel className="podcast-workspace-panel">
      <div className="podcast-studio-shell">
        <header className="podcast-studio-header"><div><p className="eyebrow">Conversation engine</p><h2 id="module-title">{module.label}</h2><p>Create a real LLM-generated podcast script, stream live preview turns through the Voice Studio TTS path, then keep one final stitched render for playback and download.</p></div><code>/podcast-renderer</code></header>
