...
function copyResampledMono(source: AudioBuffer, destination: Float32Array, offset: number, targetSampleRate: number) {
  ...
  if (source.sampleRate === targetSampleRate) {
    destination.set(sourceData.slice(0, targetLength), offset);
    return targetLength;
  }
...
async function stitchLiveOutputs(outputs: PlayablePodcastOutput[]): Promise<PlayablePodcastOutput | null> {
  ...
  try {
    ...
    const totalLength = buffers.reduce((sum, buffer) => sum + Math.max(1, Math.round(buffer.duration * sampleRate)), 0);
    const stitched = new Float32Array(totalLength);
    let offset = 0;
    for (const buffer of buffers) offset += copyResampledMono(buffer, stitched, offset, sampleRate);
...
export function PodcastWorkspace({ module }: { module: OmnixModuleDefinition }) {
  ...
  const audioRestoreRef = useRef<{ time: number; play: boolean } | null>(null);
  ...
  async function rebuildStitchedLiveOutput(outputs: PlayablePodcastOutput[]) {
    ...
    try {
      ...
      audioRestoreRef.current = { time: restoreTime, play: shouldResume };
  ...
  const createJobMutation = useMutation({
    ...
    onMutate: () => { stitchingVersionRef.current += 1; liveOutputsRef.current = []; revokeLiveStitchedUrl(); setStitchedLiveOutput(null); setLiveOutputs([]); setTranscript([]); setLiveAutoplay(false); selectOutputKey('__script__'); setDirectorNote('Generating a real speaker-tagged script with the configured chat provider.'); setActionMessage('Requesting podcast script from LLM...'); },
  ...
  function resetPodcast() { setTitle(''); setBrief(''); setDuration('5 min'); setTranscript([]); liveOutputsRef.current = []; stitchingVersionRef.current += 1; revokeLiveStitchedUrl(); setStitchedLiveOutput(null); setLiveOutputs([]); setDirectorNote('New podcast request cleared. Add a title and brief, then generate.'); selectOutputKey('__new__'); setActionMessage('New podcast ready.'); }
  ...
  function onAudioLoadedMetadata(event: React.SyntheticEvent<HTMLAudioElement>) { const audio = event.currentTarget; audio.playbackRate = Number.parseFloat(playbackRate) || 1; const restore = audioRestoreRef.current; if (!restore) return; audioRestoreRef.current = null; if (restore.time > 0 && Number.isFinite(audio.duration)) audio.currentTime = Math.min(restore.time, Math.max(0, audio.duration - 0.05)); if (restore.play) { const playResult = audio.play(); if (playResult?.catch) playResult.catch(() => setActionMessage('Audio is ready. Press play if the browser blocked autoplay.')); } }
