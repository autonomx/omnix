...
const liveRefreshTailSeconds = 0.45;
const liveRestoreSafetySeconds = 0.08;
...
export function PodcastWorkspace({ module }: { module: OmnixModuleDefinition }) {
  ...
  const pendingStitchedOutputRef = useRef<PlayablePodcastOutput | null>(null);
  const pendingStitchedApplyTimerRef = useRef<number | null>(null);
  ...
  function clearPendingStitchedOutput() { pendingStitchedOutputRef.current = null; if (pendingStitchedApplyTimerRef.current !== null && typeof window !== 'undefined') window.clearTimeout(pendingStitchedApplyTimerRef.current); pendingStitchedApplyTimerRef.current = null; }
  ...
  function applyStitchedLiveOutput(stitched: PlayablePodcastOutput) {
    clearPendingStitchedOutput();
    ...
    audioRestoreRef.current = { time: audio?.currentTime || 0, play: shouldResume, capturedAt: nowMs() };
  ...
  function applyPendingStitchedOutput() { const pending = pendingStitchedOutputRef.current; if (!pending) return false; applyStitchedLiveOutput(pending); return true; }
  function maybeApplyPendingStitchedOutput() { const audio = audioRef.current; if (!pendingStitchedOutputRef.current) return; if (!audio || audio.paused || !Number.isFinite(audio.duration) || audio.duration - audio.currentTime <= liveRefreshTailSeconds) applyPendingStitchedOutput(); }
  async function rebuildStitchedLiveOutput(outputs: PlayablePodcastOutput[]) {
    ...
    try {
      ...
      const audio = audioRef.current;
      const remaining = audio && selectedOutputKeyRef.current === stitchedLiveKey && stitchedLiveOutput && !audio.paused && Number.isFinite(audio.duration) ? audio.duration - audio.currentTime : 0;
      if (remaining > liveRefreshTailSeconds) {
        pendingStitchedOutputRef.current = stitched;
        if (pendingStitchedApplyTimerRef.current !== null && typeof window !== 'undefined') window.clearTimeout(pendingStitchedApplyTimerRef.current);
        if (typeof window !== 'undefined') pendingStitchedApplyTimerRef.current = window.setTimeout(() => { pendingStitchedApplyTimerRef.current = null; applyPendingStitchedOutput(); }, Math.max(0, (remaining - liveRefreshTailSeconds) * 1000));
        debug('deferred stitched refresh until current audio tail', { remaining, outputs: outputs.length });
        return;
      }
      applyStitchedLiveOutput(stitched);
    } catch (error) {
  ...
  useEffect(() => () => { clearPendingStitchedOutput(); revokeLiveStitchedUrl(); }, []);
  ...
  const createJobMutation = useMutation({
    mutationFn: async () => {
      ...
      setDirectorNote('Script is ready. Live preview chunks are being stitched into one growing Storyteller-style audio stream. New chunks extend the current audio without replacing it mid-sentence.');
  ...
  function resetPodcast() { clearPendingStitchedOutput(); clearAudioElement(); setTitle(''); setBrief(''); setDuration('5 min'); setTranscript([]); setLiveAutoplay(false); liveOutputsRef.current = []; stitchingVersionRef.current += 1; revokeLiveStitchedUrl(); setStitchedLiveOutput(null); setLiveOutputs([]); setDirectorNote('New podcast request cleared. Add a title and brief, then generate.'); selectOutputKey('__new__'); setActionMessage('New podcast ready.'); }
