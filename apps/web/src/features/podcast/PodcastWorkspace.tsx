...
function transcriptRowsFromSegments(segments: Array<{ speaker: string; text: string }>, targetSeconds: number): TranscriptRow[] {
  ...
  return segments.map((segment, index) => ({
    timestamp: formatClock(index * segmentStep),
  ...
  return {
    ...
    speakers: args.speakers.map((speaker) => ({
      ...
      voiceMapping: {
        ...
        voiceDisplayName: args.voiceOptions.find((voice) => voice.id === speaker.voice)?.label || speaker.voice,
        previewAvailable: Boolean(speaker.voice),
      },
    })),
    relationships: mockPodcastRelationships,
    relationship_overrides: args.relationships,
    constraints: {
      maxDurationSeconds: targetSeconds,
...
async function streamPodcastSegments(args: {
  ...
  onProgressStage: (stage: number) => void;
  ...
  if (wav && !args.abortSignal.aborted) {
    args.onProgressStage(3);
    args.onComplete({ dataUrl: wav.dataUrl, duration: wav.duration, jobId: 'live-stream', key: liveStreamKey, title: `${args.title || 'Podcast'} live preview` });
    return true;
  }
  return false;
}
...
export function PodcastWorkspace({ module }: { module: OmnixModuleDefinition }) {
  ...
  useEffect(() => {
    ...
    setSpeakers((current) => current.map((speaker, index) => (
      speaker.voice && voiceOptions.some((voice) => voice.id === speaker.voice)
        ? speaker
        : { ...speaker, voice: voiceOptions[index % voiceOptions.length].id }
    )));
  }, [voiceOptions.map((voice) => voice.id).join('|')]);

  const createJobMutation = useMutation({
    mutationFn: (segments: Array<{ index: number; speaker: string; text: string }>) => omnixApiClient.createJob({
      module: 'podcast',
    ...
    onMutate: () => {
      setDirectorNote(streamOutput ? 'Live preview complete. Rendering final persisted podcast output.' : 'Director queued final podcast rendering.');
      setStreamStatus((current) => current || 'Final podcast render queued.');
      setActionMessage('Final podcast render queued.');
    },
    onSuccess: async (job) => {
      ...
      if (rows.length) {
        setTranscript(rows);
        setStoredTranscripts((current) => {
          const next = { ...current, [job.id]: rows };
          writeStoredTranscripts(next);
          return next;
        });
      }
  ...
  const stages = liveStreamActive
    ? plannedStages.map((stage, index) => ({ ...stage, state: index < liveStreamStage ? 'done' : index === liveStreamStage ? 'active' : 'pending' }))
    : jobStages.length
      ? jobStages.map((stage, index) => ({ id: stage.id, label: stage.label, state: stageState(stage.status, index, activeStage) }))
      : plannedStages.map((stage) => ({ ...stage, state: 'pending' }));
  ...
  useEffect(() => {
    ...
    if (selectedOutputKey === '__pending__') {
      const newOutput = createJobMutation.data?.id ? playableOutputs.find((output) => output.jobId === createJobMutation.data?.id) : playableOutputs.find((output) => output.jobId === 'live-stream');
  ...
  function bufferStreamOutput(output: PlayablePodcastOutput) {
    ...
    const resume = Boolean(player && !player.paused);
    const priorTime = player?.currentTime ?? 0;
    ...
    window.requestAnimationFrame?.(() => {
      const nextPlayer = audioRef.current;
      if (!nextPlayer) return;
      if (priorTime > 0 && output.duration > priorTime) nextPlayer.currentTime = priorTime;
      if (resume) nextPlayer.play().catch(() => undefined);
    });
  }
  function startGeneration() {
    ...
    void (async () => {
      const streamed = await streamPodcastSegments({
        ...
        onProgressStage: setLiveStreamStage,
        onComplete: (output) => {
          bufferStreamOutput(output);
          setStreamStatus('Live preview complete. Final persisted podcast output is queued next.');
        },
      });
      if (abortController.signal.aborted) return;
      setLiveStreamActive(false);
      setLiveStreamStage(streamed ? 3 : -1);
  ...
  function stopLiveStream() {
    ...
    setLiveStreamActive(false);
    setLiveStreamStage(-1);
    audioRef.current?.pause();
    setStreamStatus(streamOutput ? 'Live preview stopped. Buffered audio remains in the player.' : 'Live preview stopped.');
    setActionMessage('Live preview stopped.');
  }
  function seekAudio(deltaSeconds: number) {
    const player = audioRef.current;
    if (!player) return;
    const maxTime = Number.isFinite(player.duration) ? player.duration : currentOutput?.duration ?? player.currentTime;
    player.currentTime = Math.max(0, Math.min(maxTime, player.currentTime + deltaSeconds));
  }
          ...
          <section className="podcast-studio-stack">
              ...
              <div className="episode-setup-grid">
                ...
                <div className="podcast-config-stack">
                  ...
                  <div className="podcast-select-grid">
                    <label>Duration<select value={duration} onChange={(event) => setDuration(event.target.value)}>{durationOptions.map((option) => <option key={option}>{option}</option>)}</select></label>
                  ...
                  <div className="generation-style-panel">
                    ...
                    {generationStyleOptions.map((option) => <label key={option.id} className={generationStyle === option.id ? 'generation-style selected' : 'generation-style'}><input type="radio" checked={generationStyle === option.id} onChange={() => setGenerationStyle(option.id)} /><span><strong>{option.label}</strong><small>{option.description}</small></span></label>)}
                    <div className="review-stop-row">{reviewStopOptions.map((option) => <label key={option.id}><input type="checkbox" disabled={generationStyle !== 'guided'} checked={manualReviewStops.includes(option.id)} onChange={() => toggleReviewStop(option.id)} />{option.label}</label>)}</div>
                  </div>
                </div>
              </div>
            </article>
            <article className="podcast-card">
              ...
              <div className="speaker-table editable-speaker-table">
                ...
                {speakers.map((speaker) => <div className="speaker-row editable-speaker-row" key={speaker.id}>
                  ...
                  <span><select aria-label={`${speaker.name} voice`} value={speaker.voice} onChange={(event) => updateSpeaker(speaker.id, 'voice', event.target.value)}>{voiceOptions.map((voice) => <option key={voice.id} value={voice.id}>{voice.label}</option>)}{!voiceOptions.length ? <option value="">No cloned voices</option> : null}</select></span>
            ...
            <article className="podcast-card relationship-card">
              <h3>3. Relationships and constraints</h3>
              <div className="relationship-layout relationship-layout-clear">
                <div className="relationship-map relationship-map-clear">
                  <div className="relationship-map-title"><b>Panel relationships</b><small>Directional links for the current episode</small></div>
                  <div className="relationship-node-card host"><b>H</b><span>{relationships.hostLabel}</span><small>moderator</small></div>
                  <div className="relationship-node-card guest-a"><b>GA</b><span>{relationships.guestALabel}</span><small>guest A</small></div>
                  <div className="relationship-node-card guest-b"><b>GB</b><span>{relationships.guestBLabel}</span><small>guest B</small></div>
                ...
                <div className="relationship-config-grid"><label>Host label<input value={relationships.hostLabel} onChange={(event) => updateRelationship('hostLabel', event.target.value)} /></label><label>Guest A label<input value={relationships.guestALabel} onChange={(event) => updateRelationship('guestALabel', event.target.value)} /></label><label>Guest B label<input value={relationships.guestBLabel} onChange={(event) => updateRelationship('guestBLabel', event.target.value)} /></label><label>Moderator relation<input value={relationships.moderation} onChange={(event) => updateRelationship('moderation', event.target.value)} /></label><label>Respect relation<input value={relationships.respect} onChange={(event) => updateRelationship('respect', event.target.value)} /></label><label>Conflict relation<input value={relationships.disagreement} onChange={(event) => updateRelationship('disagreement', event.target.value)} /></label></div>
                <div className="constraint-grid editable"><label><small>Max duration</small><strong>{durationClock(duration)}</strong></label><label><small>Citation required</small><select value={citationRequired} onChange={(event) => setCitationRequired(event.target.value)}><option>On</option><option>Off</option></select></label><label><small>Family friendly</small><select value={familyFriendly} onChange={(event) => setFamilyFriendly(event.target.value)}><option>On</option><option>Off</option></select></label><label><small>Reading level</small><select value={readingLevel} onChange={(event) => setReadingLevel(event.target.value)}><option>Grade 8</option><option>Grade 10</option><option>Expert</option></select></label><label><small>Max turn</small><select value={maxTurnSeconds} onChange={(event) => setMaxTurnSeconds(event.target.value)}><option value="20">20 sec</option><option value="45">45 sec</option><option value="60">60 sec</option><option value="90">90 sec</option></select></label><label><small>Avoid topics</small><input value={avoidTopics} onChange={(event) => setAvoidTopics(event.target.value)} /></label></div>
              </div>
            </article>
