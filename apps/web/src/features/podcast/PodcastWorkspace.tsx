...
export function PodcastWorkspace({ module }: { module: OmnixModuleDefinition }) {
  ...
  const createJobMutation = useMutation({
    ...
    onSuccess: async (job) => {
      if (isFailed(job.status)) {
        ...
        setActionMessage(`Podcast production queued: ${job.id}`);
      }
  ...
  const previewVoiceMutation = useMutation({
    ...
    onSuccess: async (job) => { setActionMessage(isFailed(job.status) ? jobErrorMessage(job) : `Voice preview queued: ${job.id}`); await Promise.all([queryClient.invalidateQueries({ queryKey: ['platform', 'jobs'] }), queryClient.invalidateQueries({ queryKey: ['platform', 'assets'] })]); },
  });
